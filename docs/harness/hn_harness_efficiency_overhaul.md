...
- [x] Windows·Git Bash 환경 호환 검증 — Git Bash 실측 통과. PowerShell·WSL은 운용 검증 필요 (자동 검증 불가, 별 WIP)
...
- [x] eval/SKILL.md — CPS 무결성 감시는 별 WIP로 분리 — `decisions--hn_eval_cps_integrity.md`
- [x] commit 스킬 5.3 자동 실행 코드는 별 WIP로 분리 — `decisions--hn_commit_auto_verify.md`. 1단계에 SSOT 정의 완료
...
**Acceptance Criteria** (전부 폐기 — Phase 2-A AC + CPS로 흡수):
- [x] 폐기됨 — fast-path white-list 자체가 외형 metric. 사용자 지적으로 AC + CPS로 전면 재정의
- [x] 분류는 작성자 자가 보고 (검증.review skip|self|review|review-deep) 그대로 stage 결정 — Phase 2-A에서 구현
- [x] 외형 metric 룰 (UPSTREAM_PAT·docs 5줄·rename 단독 등) 모두 폐기됨 — Phase 2-A 2단계 v0.29.1
- [x] 핵심 가드 유지 — 시크릿 line-confirmed → deep 보안 게이트, frontmatter 누락 차단
...
**Acceptance Criteria** (별 WIP로 분리):
- [x] 본 wave 밖 — `decisions--hn_downstream_amplification.md`로 분리. baseline 측정 게이트 + 게이팅 코드 통합 추적
...
### 2026-05-02 — Phase 1·2-A·3 완료, 별 wave 분리

**완료된 작업** (atomic commits 5개):
- Phase 1 시크릿 hook 이중화 (v0.28.8 — `3c888f2`·`bbe370f`·`4b3854c`): 안전망 5/10 → 7.5/10
- Phase 3 split 옵트인 강등 (v0.28.9 — `5a4ea57`): 5/5 skip 케이스 자동 single
- Phase 2-A 1단계 시스템 정의 (v0.29.0 — `6488cb6`): docs.md·staging.md·룰 12개·에이전트 9개·SKILL 3개 + frontmatter SSOT
- Phase 2-A 2단계 시스템 강제 (v0.29.1 — `ebee1c7`): pre_commit_check 박제 감지·외형 metric 폐기. 자기증명 통과

**별 wave 분리**:
- `decisions--hn_eval_cps_integrity.md` — eval CPS 무결성 감시
- `decisions--hn_commit_auto_verify.md` — commit 5.3 자동 실행 코드
- `decisions--hn_downstream_amplification.md` — Phase 4 (측정 게이트 필수)
- `decisions--hn_rule_skill_ssot.md` — Phase 5 룰-스킬 중복 제거

**구 Phase 2 fast-path**: 외형 metric 자체가 자기모순으로 폐기 — Phase 2-A AC + CPS로 재정의돼 흡수.

**자기증명 통과**: 본 wave 변경(staging·pre-check·review)이 본 WIP frontmatter·AC를 직접 검증 → 본 commit이 새 시스템 통과. 5/5 skip 사례 본 wave에서 0건 발생.

CPS 갱신: 없음 (P2·P5 진전 — Solution 충족 기준 변경 없음, 메커니즘만 강화).
