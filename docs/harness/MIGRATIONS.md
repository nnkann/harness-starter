...
## v0.28.0 — 테스트 다이어트: AC 트리거 + marker + 다운스트림 격리

### 변경 파일
| 파일 | 처리 | 비고 |
|------|------|------|
| `.claude/scripts/test_pre_commit.py` | 3-way merge | 5개 영역 marker 부여 (secret 3 / gate 2 / stage 8 / enoent 12 / docs_ops 15). docstring에 트리거 원칙 명시 |
| `.claude/scripts/conftest.py` | 신규 | pytest marker 등록 (warning 방지) |
| `.claude/rules/self-verify.md` | 3-way merge | "검증 항목"에서 "기존 테스트 통과 매번" 제거. "테스트 판단" 섹션을 트리거 매트릭스 + marker 목록으로 재작성 |
| `.claude/rules/docs.md` | 3-way merge | AC 포맷에 `pytest -m <marker>` 표현 명문화. "테스트 트리거" 절 추가 |
| `.claude/skills/harness-dev/SKILL.md` | 3-way merge | 체크리스트 "테스트" 항목 — "AC가 요구하면 marker만" 트리거로 변경 |
| `.claude/skills/implementation/SKILL.md` | 3-way merge | "테스트 스위트" 항목 — "AC가 요구할 때만" 트리거로 변경 |
| `h-setup.sh` | 자동 덮어쓰기 | `STARTER_ONLY_SCRIPTS` 제외 로직 (upgrade·신규 설치 두 경로). upgrade에 기존 cleanup 동작 추가 |

### 적용 방법
자동. `harness-upgrade` 실행 시 3-way merge + h-setup.sh 자동 덮어쓰기.

### 주의
- **다운스트림 cleanup**: harness-upgrade 후 다운스트림에 `test_pre_commit.py`·`conftest.py`가 자동 제거된다. 다운스트림이 직접 작성한 테스트는 별도 파일명으로 유지하면 영향 없음.
- **트리거 변경**: 기존엔 `pre_commit_check.py` 변경 시 무조건 pytest 16초. 이제 AC가 명시 안 하면 0초. 회귀 가드 가치 있는 변경은 AC `영향 범위:`에 `pytest -m <marker>` 표현 추가.
- **marker 5종**: secret(시크릿 스캔), gate(차단 룰), stage(stage 결정·move commit), enoent(린터 ENOENT 정규식), docs_ops(dead link/relates-to/wip-sync). 영역별 실행: `pytest -m gate`.
- **사용자 명시 트리거**: `pytest`(전체), `pytest -m <marker>`(영역). CI / `eval --deep` / 사용자 요청 한정.

### 검증
```bash
# 영역별 marker 실행 (예: 차단 룰 회귀 체크)
pytest .claude/scripts/test_pre_commit.py -m gate

# 다운스트림 격리 확인 (다운스트림 repo에서)
ls .claude/scripts/test_pre_commit.py 2>&1   # 없어야 함
```
