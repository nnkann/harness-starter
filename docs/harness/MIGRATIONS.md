...
## v0.44.1 — 커밋 파이프라인 route 출력·side effect ledger·Windows smoke (2026-05-13)

### 변경 내용

`hn_commit_perf_optimization` wave를 한 번에 통과시켜 커밋 파이프라인을
CPS·AC 기준의 fast/release 경로와 명시적 side effect 분류로 재편했다.

- `pre_commit_check.py`: stdout에 8개 route 키 추가 — `commit_route`,
  `review_route`, `promotion`, `blocking_reasons`, `warning_reasons`,
  `side_effects.{required,release,repair}`. `review_route`는
  `recommended_stage` 기반이며, docs-only + 시크릿 없음 + promotion=none +
  WIP ≤1일 때 정량 강등 (`standard|deep` → `micro`)
- `pre_commit_check.py`: `get_secret_patterns()` SSOT 함수 + `--emit-secret-patterns`
  서브커맨드. 본 모듈 내부 시크릿 스캔을 SSOT 호출로 일원화. hook 설치
  스크립트 마이그레이션은 별 wave
- `pre_commit_check.py`: `warning_reasons`에 cascade integrity 신호 누적
  (`cluster-abbr-unknown`·`cascade-defends-missing`·`cascade-serves-trigger-missing`).
  ERRORS는 건드리지 않음 (경고 only)
- `docs_ops.py wip-sync`: stdout에 `wip_sync_updated`·`cluster_updated`·
  `backrefs_updated` 추가 (기존 `wip_sync_matched`·`wip_sync_moved` 보존).
  `docs_ops.py move`는 `backrefs_updated: N` alias 추가
- `commit_finalize.sh`: wip-sync 출력을 캡처해 `## side_effects (commit_finalize)`
  블록 + `side_effects.required: <항목>=<카운트>` 포맷으로 재출력
- `split-commit.sh`: 비파괴 planner로 전환. 기본은 plan 출력만, `--apply`
  명시 시에만 staged 비우고 첫 그룹 stage. split-plan.txt 자동 다음 그룹
  흐름은 보존
- `.claude/skills/commit/SKILL.md` (+ `.agents/skills/commit/SKILL.md`):
  Pass 표 + pre-check stdout 형식 + Step 4·5.5·7 + 사용자 노출 알림 +
  최종 요약을 route 출력 소비 방식으로 재작성
- `bash-guard.sh`: 워킹트리 CRLF 156개 → LF 정규화 (Windows + Git Bash
  실행 안정성)
- `test_pre_commit.py`: `TestRouteOutput` (4) + `TestWindowsCommitSmoke` (3) +
  `TestCascadeIntegrity` (2) 9개 회귀 테스트 추가. windows·cascade marker 신설

### 적용 방법

자동. harness-upgrade로 fetch 후 다음 `/commit` 시 새 route 출력이 활성화된다.

### 검증

- `pytest -m "stage or windows or cascade"` — 16 passed
- 다운스트림 영향 (CRLF·hook 시크릿 SSOT 마이그레이션 등) 평가는 별 wave 진행 예정

### 회귀 위험

upstream 격리 환경(Windows + Git Bash)에서 76 passed / 2 failed 측정.
실패 2건은 본 wave 변경 전 기준선에서도 동일 실패하는 `TestCommitFinalize`
2종 (`fatal: could not parse HEAD` — Windows path 혼합 이슈, BIT IGNORE).
Linux/macOS 환경은 미테스트.

`review_route` 강등은 객관 신호 4종(stage in {standard,deep}, s1_level ≠
line-confirmed, promotion = none, all .md + WIP ≤1) 동시 만족 시에만
발동하도록 정량화. 다운스트림 코드 변경 커밋에는 영향 없음.
