...
## v0.27.5 — pre_commit_check task 단위 AC scope + review diff 조건 명시

### 변경 파일
| 파일 | 처리 | 비고 |
|------|------|------|
| `.claude/scripts/pre_commit_check.py` | 3-way merge | 6번 섹션 — staged 파일을 task의 `impact_files`에 매칭한 뒤 그 task의 kind/has_impact_scope만 사용. WIP 파일 전체 스캔 폐지 |
| `.claude/scripts/task_groups.py` | 3-way merge | `parse_wip_tasks()` 신설 — task 단위 `{kind, impact_files, has_impact_scope}` 반환. `parse_wip_impact()`는 후방 호환 wrapper 유지 |
| `.claude/agents/review.md` | 3-way merge | 라인 72에 diff 실행 조건 3개 명시 (스코프 이탈 의심·Read 특정 불가·AC pass 시 불필요). 도구 선택 표 재배치 (AC 충족 첫 행, diff 마지막 조건부) |

### 적용 방법
자동. `harness-upgrade` 실행 시 3-way merge로 적용.

### 주의
- v0.27.4 도입 직후 발견된 두 설계 구멍에 대한 후속 수정. 단독 v0.27.4 적용 환경은 권장하지 않음 (Task 1만 staged해도 false deep 발생).
- `parse_wip_impact()` 시그니처는 변경 없음. 기존 호출자(`task_groups.py main`) 영향 없음.
- review 동작 변화: AC 영향 파일과 staged 파일이 일치하면 `git diff --cached` 호출 없이 통과 — tool call 1회 절감.

### 검증
```bash
python3 -m pytest .claude/scripts/test_pre_commit.py -q
```
