...
- 기존 (A) DELETED 카테고리에 더해 **(B) starter_skills 격하 잔재 감지**
  추가. upstream `HARNESS.json.starter_skills`에 등록된 폴더가 다운스트림
  디스크에 존재하면 격하 잔재로 감지 → 사용자에게 삭제 제안
- starter 본인(`is_starter: true`)은 (B) 검사 skip — 자기 파일은 잔재 아님
