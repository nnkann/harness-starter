...
## v0.27.4 — AC 기반 commit 흐름 전환: 신호 체계 제거 + review AC 중심 재작성

### 변경 파일
| 파일 | 처리 | 비고 |
|------|------|------|
| `.claude/scripts/pre_commit_check.py` | 3-way merge | S1~S15 신호 판정·stage 결정 로직 제거. WIP kind 읽기 + AC kind 기반 stage 판단으로 교체. 시크릿 스캔 유지 |
| `.claude/scripts/test_pre_commit.py` | 3-way merge | 신호 판정 테스트 제거 (56→35개). ENOENT·dead link·relates-to·move commit·wip-sync 테스트 유지 |
| `.claude/rules/staging.md` | 3-way merge | S1~S15 신호 체계 전면 제거. AC kind 기반 stage 판단 규칙으로 교체 |
| `.claude/skills/commit/SKILL.md` | 3-way merge | diff 전처리·전달 블록 제거. WIP AC 전문 박기 방식으로 교체. pre-check stdout 스키마 변경 |
| `.claude/agents/review.md` | 3-way merge | diff 기반 독립 판단 → AC 항목 체크 중심으로 전면 재작성 |
| `.claude/rules/docs.md` | 3-way merge | 배경 텍스트 다이어트 (297→223줄). 강제 규칙 손실 없음 |

### 적용 방법
자동. `harness-upgrade` 실행 시 3-way merge로 적용.

### 주의
- **pre_commit_check.py stdout 스키마 변경**: `signals`, `domains`, `domain_grades`, `multi_domain`, `repeat_count`, `risk_factors` 키 제거. `wip_kind`, `has_impact_scope` 키 추가. commit 스킬이 이 키를 참조하므로 동시 업그레이드 필수.
- **commit 흐름 변경**: review 호출 시 diff 전달 없음. WIP AC 전문을 prompt에 박는 구조로 전환. WIP 없이 커밋 시 review가 AC 없이 동작 (계약·스코프 2축만).
- **gitleaks 미설치 환경**: pre_commit_check.py의 시크릿 스캔이 1차 방어 유지. line-confirmed 패턴 감지 시 커밋 차단 동작 동일.

### 검증
```bash
python3 -m pytest .claude/scripts/test_pre_commit.py -q -k "not WipSync"
```
