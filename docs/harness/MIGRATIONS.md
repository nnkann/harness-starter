...
## v0.27.6 — completed 전환 차단 로직 정밀화

### 변경 파일
| 파일 | 처리 | 비고 |
|------|------|------|
| `.claude/scripts/docs_ops.py` | 3-way merge | BLOCK_ITEMS 정규식 제거. TODO/FIXME 콜론 표기·빈 체크박스·미결 섹션 헤더만 차단 |
| `.claude/rules/docs.md` | 3-way merge | "completed 전환 차단" 섹션 — 키워드 목록 → 명령형 패턴 3종으로 재서술 |

### 적용 방법
자동. `harness-upgrade` 실행 시 3-way merge로 적용.

### 주의
- 기존 룰: 본문에 `후속`·`미결`·`추후` 등 키워드 단독 매칭 → 회고적 서술도 차단됐음
- 새 룰: TODO:/FIXME: 콜론·빈 체크박스 `- [ ]`·`## 후속` 섹션 헤더만 차단
- 다운스트림 영향: completed 전환 시도가 단어 단독으로 차단되던 문서들 → 자동 통과 가능

### 검증
```bash
python3 -c "import sys; sys.path.insert(0, '.claude/scripts'); from docs_ops import BLOCK_KEYWORDS, BLOCK_EMPTY_CHECKBOX, BLOCK_HEADERS"
```
