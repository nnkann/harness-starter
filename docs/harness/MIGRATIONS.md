...
## v0.55.1 — WIP/Kanban 통합 정책과 frontmatter 검증 보강 (2026-06-06)

### 변경 내용

- WIP 문서 계약과 Hermes Kanban 실행 상태의 책임을 분리하는 통합 정책을 추가했다.
- `docs_ops.py` frontmatter 검증이 PyYAML 없는 최소 Python 환경에서도 동작하도록 fallback parser를 보강했다.
- `eval --harness`에 policy/dispatcher drift 관측 항목을 추가했다.
- runtime adapter 표면과 `minimal` 프로파일 의미를 루트 안내와 설치 출력에 명시했다.
- worktree blanket ban을 소유권·정리·권한 계약으로 대체하고, sandbox 검증은 permission-ready 조건에서만 완료 증거로 쓰도록 정리했다.
...
- `.claude/scripts/tests/test_docs_ops_staging.py`
- `.claude/scripts/tests/test_eval_harness.py`
- `.claude/scripts/tests/test_h_setup_runtime_metadata.py`
...
- `AGENTS.md`, `CLAUDE.md`, `README.md`, `h-setup.sh`

### 수동 적용 항목

- 다운스트림에서 Hermes Kanban과 legacy `docs/WIP`가 함께 쓰이면
  `docs/harness/hn_wip_kanban_integration_policy.md`를 기준으로 기존 WIP와 board task를
  operational-only, doc-contract-required, invalid-intake, already-covered로 분류한다.
- PyYAML이 없는 환경에서도 `docs_ops.py validate`는 동작해야 한다. 별도 PyYAML 설치를 요구하지 않는다.
- worktree는 blanket ban 대상이 아니다. 생성 시 owner, 목적, 정리 조건, 변경 보존 방식을 명시한다.

### 검증

- `python3 .claude/scripts/docs_ops.py validate`
- `python3 .claude/scripts/safe_command.py verify-relates`
- `python3 .claude/scripts/safe_command.py eval-harness`
- `python3 -m py_compile .claude/scripts/docs_ops.py .claude/scripts/eval_harness.py .claude/scripts/safe_command.py`
- `bash -n h-setup.sh .claude/scripts/bash-guard.sh .claude/scripts/test-bash-guard.sh`
- `bash .claude/scripts/test-bash-guard.sh`

### 회귀 위험

- 중간. `docs_ops.py` fallback parser는 Harness frontmatter의 scalar, inline list,
  간단한 block list 범위를 지원한다. 복잡한 YAML 구조를 새로 도입하는 downstream은
  PyYAML 환경에서 추가 검증하거나 frontmatter를 단순 구조로 유지해야 한다.
- WIP/Kanban 통합 정책은 Harness SSOT를 강화하지만, Hermes runtime dispatcher 설정은
  별도 control-plane 적용이 필요하다.
...
1. **Kanban/WIP 중복 처리**: 같은 요청이 Kanban task와 `docs/WIP`에서 서로 다른 완료 상태로 남은 사례가 있었는가?
   - 응답 예: "N건 발생 / 0건 / 미관측"
2. **frontmatter fallback**: PyYAML 없는 환경에서 `docs_ops.py validate`가 정상 동작했는가?
   - 응답 예: "정상 / 실패 로그 첨부 / 미관측"
