---
title: harness-starter CPS (73% 삭감 wave 이전 박제)
domain: harness
tags: [cps, meta, starter, archived]
status: abandoned
created: 2026-04-20
updated: 2026-05-14
---

> **2026-05-14 박제**: 73% 삭감 wave에서 kickoff을 C 판단 프롬프트(자라지
> 않음, 짧게)로 재정의. 본 문서는 그 이전 491줄 본문 보존용. P1~P9 정의·
> Solution·Anti-Pattern·Constraints 모두 본문에 보존.
...
**진입 조건** (객관 신호):
- 동일 파일이 같은 세션에서 3회 이상 수정 (git history grep)
- pre_commit_check.py "핵심 설정 연속 수정 차단" 게이트 hit
- 같은 에러·테스트 실패가 2회 이상 반복 발생 (tool 결과 패턴 매칭)
- debug-specialist 호출 흔적 없이 동일 에러 메시지 후속 응답에 재출현
...
**진입 조건** (객관 신호):
- review 호출의 tool call 수가 stage 상한 초과 (micro >2, standard >4, deep >5)
- AC `검증.review: skip`·`self` 선언인데 review 응답 길이 > 임계
- pre-check `recommended_stage: deep`이 docs-only 변경에서 발생 (staging.md 룰 4번 외)
- maxTurns 소진으로 verdict 누락 (extract_review_verdict.py exit 1)
...
**진입 조건** (객관 신호):
- `harness_version_bump.py` minor/patch 판정인데 MIGRATIONS.md 새 섹션 staged 0
- `.claude/permissions.allow` 신규 항목 staged인데 MIGRATIONS.md 수동 액션 명시 없음
- `starter_skills/` 영역 staged 파일이 다운스트림 배포 흐름과 불일치
- downstream-readiness.sh 검증 hit (다운스트림 머신에서 실행 시)
...
**진입 조건** (객관 신호):
- `.claude/settings.json` `hooks` 블록 `matcher`·`if`에 argument-constraint
  패턴 staged (`Bash(* --* *)`, `Bash(git * -n*)` 등 — `rules/hooks.md` 정규식 SSOT)
- bash-guard.sh stderr 차단 로그가 같은 명령에서 N회 반복
- 정당 명령에 대한 hook 차단 사용자 보고 (incident grep)
...
**진입 조건** (객관 신호):
- 서브에이전트 호출 응답에 컨텍스트 토큰 수 보고 > 임계
- SKILL.md·rules 파일 줄 수 > 500 (eval --harness MVR 미적용 영역)
- review·researcher 응답에서 후반부 규칙 인용률 < 임계 (미완독 패턴)
- MCP 서버 활성 상태 + 응답 지연 측정
...
**진입 조건** (객관 신호):
- staged diff에 `.claude/skills/**/SKILL.md` 또는 `.claude/rules/*.md` 변경
  있는데 AC `검증.tests: 없음` 선언
- CPS Solution 메커니즘 변경 staged인데 MIGRATIONS.md 갱신 없음
- pre-check `recommended_stage` deep 격상 hit인데 review 호출 0
- 핵심 스크립트(`pre_commit_check.py`·`docs_ops.py`) 변경 staged인데 pytest
  marker 실행 흔적 0
...
**진입 조건** (객관 신호):
- 신규 rules/skills/agents/scripts 파일 frontmatter `defends:`·`serves:`
  필드 누락
- HARNESS_MAP.md에 등록 안 된 도구 (frontmatter는 있는데 MAP CPS 테이블
  누락) — eval_cps_integrity 매핑 검증 hit
- frontmatter `defends:` 인용 P# 가 CPS Problems 목록 미존재
- 단일 진입점 없는 작업유형 (MVR 미정의)
...
**진입 조건** (객관 신호):
- 신규 rules 파일 본문에 "Claude가 ~", "에이전트가 ~ 발화" 등 자가 의존
  명령형이 있는데 hook 또는 pre-check 강제 메커니즘 부재
- 다운스트림 환경에서 특정 rules SSOT 인용 git log 카운트 0 (배포 후 N일)
- starter incidents/ 파일이 N일 후 재발 사례 보유 (회상 의존 실패)
- 자가 마킹·자가 보고에만 의존하는 흐름 (예: AC 체크박스 자가 ✅)
  통계상 누락률 > 임계 (commit log audit)
