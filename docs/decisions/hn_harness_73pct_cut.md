...
영역 7: Wiki 그래프 모델 (§S-7)         [완료 7d00a6d, §S-5·§S-6 분산]
영역 8: AC 체크박스 게이트 (§S-8)       [완료 본 commit]
영역 9: CPS cascade 정합 게이트 (§S-9) [완료 본 commit]
...
## S-9. CPS cascade 정합 게이트 (S# → AC 강제)

**defends**: P6 (검증망 스킵 패턴), P9 (정보 오염 관성)
**serves**: S6 (4중 방어 강화), S9 (주관 격리 다층 검증)

> 2026-05-15 본 wave 마감 단계 발견. 사용자 우려: "S#를 선택했으면 그에
> 합당한 AC가 나와야지. 임의로 툭 튀어나온 AC가 아니라."

### 본질 — S# → AC cascade 단절 해소

기존 흐름의 갭:
```
CPS Problem 매칭 → Solution 인용 (S# 번호만)
  ↓ ← 단절
WIP frontmatter problem: P# / s: [S#]
  ↓
AC 작성 (S#와 무관해도 통과)
```

→ 임의 AC = 학습 신호 위장. S#가 AC SSOT여야 cascade 완결.

### 두 함정 사이의 좁은 길

| 함정 | 본 §S-9 회피 |
|------|------------|
| §S-1 50자 substring 박제 (wave가 S# 본문 자라면 박제 단절) | ❌ substring 매칭 X — 번호만 |
| 자가 발화 의존 (rule 명시만, 결정적 차단 0) | ❌ pre-check 결정적 게이트 |
| 매칭 강제로 의미 왜곡 | ❌ 의미 검사 X. 번호 존재만 |
| 너무 유연해서 다 빠져나감 | ❌ `s: []` 빈 list 차단 — 학습 데이터 의무 |

= **번호 매칭 + 충족 기준 SSOT 박제 (자라는 시스템 유지)**.

### kickoff `## Solutions` 표 — "해결 기준" 컬럼 신설

S1~S9 각 해결 기준 1줄 박제 (archived 본문에서 추출). AC `검증.실측`
작성 시 SSOT 참조 의무. 단 substring 본문 인용 금지 (자라는 시스템).

### P10·S10 — catch-all 신설

**박는 조건 (엄격)**:
- P1~P9 각각 검토 후 어디에도 명확히 안 맞을 때만
- "잘 모르겠음·귀찮음·빠르게 넘기고 싶음" 부적합 — P10은 도피처 아님
- 의심 근거 1줄 박제 의무 + 가장 가까운 후보 P#·S# 동반 권장

**P10 본질**: 학습 시스템의 관찰 큐. `docs_ops.py cps cases --p P10`로
누적 패턴 조회 → 정련 후보. **혼용 시 본질 신호 희석 — 신중 사용**.

### pre-check 게이트 (결정적)

```python
1. frontmatter `s:` 비어있음 → 차단 (학습 데이터 입력 의무)
2. AC 섹션에 각 S# 번호 1개 이상 등장 검사 → 미등장 시 차단
3. 자기 변경 면제: kickoff·cps_master·docs/cps/* staged면 skip (순환 차단)
4. P10 인용 시 ℹ️ 안내 (차단 X) — 엄격 기준 + 후보 동반 권장
```
...
| 작업 | 결과 |
|------|------|
| kickoff `## Problems`에 P10 추가 | ✅ |
| kickoff `## Solutions`에 "해결 기준" 컬럼 + S10 추가 | ✅ |
| pre_commit_check.py 게이트 신설 (S# 번호 매칭 + P10 안내 + 자기 변경 면제) | ✅ |
| docs.md AC 포맷 박제 강화 (S# 인용 강제 + substring 인용 금지 + P10 엄격 기준) | ✅ |
| test_pre_commit.py `@pytest.mark.gate` 5 케이스 신설 | ✅ (46 passed) |

### 별 wave 후보 박제 (cps_master 이름 변경)

`project_kickoff.md` → `cps_master.md` 별 wave. 근거:
- 현 이름 "프로젝트 시작 시점"이라는 시간적 의미인데 실제로는 프로젝트
  생애 내내 자라는 SSOT. 이름·역할 불일치
- cps cascade의 마스터 — 본질 직설 이름 적정
- cascade 영향 큼 (`check_init_done.sh` hardcoded·rules·skills 참조·
  다운스트림 자기 파일과 충돌). 별 wave 박제 정합

본 wave에서는 이름 변경 X. 후보 박제만.

## 단기 결과 — 이전(migration) 효과 5축 증명

> 라인 수 단일 축은 부분 진실. 본 wave 본질은 "자가 발화 의존(rules/skills 본문)
> → 결정적 게이트(scripts)" **이전(migration)** 이므로 5축 비교가 정확한 판정.

| 축 | before (v0.46.2 2a5fcd0) | after (v0.47.7) | 신호 |
|---|---|---|---|
| **A. 시스템 프롬프트 적재** (rules+skills 라인) | 6,826 | **4,446** | **-34.9%** ✓ 매 세션 토큰 비용 실삭감 |
| **B. 결정적 차단 작동** (v0.47.0~ commit 23건 중 게이트 보정) | 0건 (자가 발화는 차단 자체 없음) | **4건** (FR-X1 false positive 축소·격하 감지·격하 잔재 강제 삭제·cascade 화이트리스트) | ✓ 게이트가 실제로 차단했고, 너무 잘 잡았을 때 정밀화한 흔적이 git에 박힘 |
| **C. 회귀 보호 커버리지** (게이트 marker) | gate 0·tag 0 | **gate 6 / tag 3 / docs_ops 8** (TestSCascade 8·TestACCheckbox 4·Sealed 3·HarnessDev 3·tag 정규식 3 등) | ✓ 게이트 약화 시 즉시 test fail — 자가 발화 룰은 약화 visible X |
| **D. 라인 수 (참고)** | rules 1,839 / skills 5,124 / agents 1,583 / scripts 2,664 = 11,073 | rules **957** (-48%) / skills **3,489** (-32%) / agents 1,434 (-9%) / scripts **5,259** (+97%) = 11,139 | 총량 -0.6% — **이전이 본질, 삭감이 아님** |
| **E. 다운스트림 cascade 마찰** (v0.47.2~v0.47.7 운용) | — | 격하 잔재 자동 삭제 1건·새 게이트(AC 체크박스·S# 인용) 마찰 0건·wiki 자산 wave(v0.47.5) 완주 | ✓ 새 게이트가 정상 흐름 차단 안 함 |

### 게이트 효과 검증 — 메커니즘

신설 게이트 4종(`pre_commit_check.py`):

| 게이트 | 위치 | 자가 발화 대체 |
|---|---|---|
| tag 정규식 (`^[a-z0-9][a-z0-9-]*[a-z0-9]$`) | :825 | 이전: docs.md "tag는 소문자 권장" (차단력 0) → 지금: commit 차단 + `auto-fix: docs_ops.py tag-normalize` |
| AC 체크박스 형식 | :856 | 이전: 자유 텍스트 AC 허용 → 지금: `- [ ]`/`- [x]` 필수. `docs_ops.py move` 빈 체크박스 완료 판정 게이트와 페어 |
| S# cascade (번호 매칭) | :874 | 이전: 50자 substring 박제 (§S-1 함정) → 지금: 경계 매칭 (`S2`≠`S20`), 자기 변경 면제 |
| 격하 잔재 강제 삭제 | harness-upgrade Step 7 | 이전: 다운스트림 starter 전용 스킬 잔재 누적 → 지금: cascade 차단 |

### 회귀 보호 메커니즘

- `@pytest.mark.gate` 6 케이스·`@pytest.mark.tag` 3 케이스·`@pytest.mark.docs_ops` 8 케이스가 신설 게이트와 정확히 페어링
- TestSCascade는 substring 미매칭(`S2` in 본문≠인용)·경계(`S2`≠`S20`) 같은 false positive 회귀 9 케이스 보호
- **게이트 약화 = 즉시 test fail** → 자가 발화 룰의 "약화돼도 약화 자체가 invisible" 문제 해소

### 다이어트 필요성 판단

A·B·C·E 모두 양성. D만 음성 (총량 동결). 추가 다이어트는 둘 중 하나:
- (a) 게이트 폐기 → P8 자가 발화 의존 복귀 (본 wave 명제 무너짐) ❌
- (b) pre_commit_check 모듈 분리·중복 제거 → 실효익 미미

→ **현 상태가 맥락 부합**. scripts 팽창은 비용이 아니라 "이전의 형상 그 자체".
"있어야 되는 걸 빼는 것" 회피 = 현 상태 유지가 정답.
...
| AC 체크박스 형식 강제 | pre-check 결정적 차단 (자유 텍스트 AC 금지) | §S-8 |
| S# → AC 인용 강제 | pre-check 번호 매칭 (substring X, §S-1 함정 회피) | §S-9 |
| kickoff Solutions 표 "해결 기준" 컬럼 | S1~S9 SSOT 박제 + S10 신설 | §S-9 |
| P10·S10 catch-all 신설 | 엄격 기준 (도피처 아님). 본질 의심 박제 — 학습 관찰 큐 | §S-9 |
| `s: []` 빈 list 금지 | 모든 wave는 S# 1개 이상 박제 (학습 데이터) | §S-9 |
| cps_master 이름 변경 | 별 wave 후보 (cascade 영향 큼) | §S-9 |
...
**Acceptance Criteria**:
- [x] Goal: 하네스 73% 삭감 — S2·S5·S6·S7·S8·S9 cascade 충족.
  - 통제 항목 폐기 (자가 발화 의존 rules·스킬·스크립트 일소)
  - CPS·AC·B 그룹·스킬·rules·스크립트·wiki 그래프·AC 게이트·S# cascade 9영역 순서 처리 완료
  - 사용자 운용 1~3 세션 체감 OK 판정으로 wave 통과
  검증:
    tests: 없음
    실측: 운용 검증
- [x] §S-1·§S-2 완료 (e998278)
- [x] §S-3·§S-4·§S-5·§S-6·§S-7 완료 (7d00a6d)
- [x] §S-8·§S-9 완료 (3213a07 v0.47.4)
- [x] 다운스트림 1회 harness-upgrade 실행 — 격하 잔재 자동 삭제 검증 (cc53471 v0.47.3, 9770499 v0.47.2)
- [x] starter 본인 다음 wave 1건 실행 — 새 게이트 마찰 vs 학습 신호 판정 (bbcd1f0 v0.47.5 wiki 자산 wave 완주, 374143a v0.47.6 Step 11 false positive 축소·tag-normalize 신설, 66813de v0.47.7 회고 잔재 자동 정리)
- [x] 누적 통계 박제 — 본 wave §단기 결과 표 실측 컬럼 추가 (scripts/ 팽창 박제 + 후속 wave 분리 근거)
...
- 2026-05-15 §S-8·§S-9 추가 박제:
  - §S-8: AC 체크박스 형식 강제 — pre-check 결정적 차단. 자유 텍스트 AC 금지.
    완료 판정 게이트(docs_ops.py move 빈 체크박스)가 작동하려면 체크박스 필수
  - §S-9: S# → AC cascade 정합 게이트 — pre-check 번호 매칭 (substring X).
    kickoff Solutions 표에 "해결 기준" 컬럼 신설 + P10·S10 catch-all 추가
    (엄격 기준, 학습 관찰 큐). `s: []` 빈 list 금지. 자기 변경 면제
    (kickoff·cps_master·docs/cps/* staged 시 skip)
  - 사용자 핵심 우려: "임의 AC 차단 + 매칭에만 공들이는 §S-1 함정 회피
    + 너무 유연한 회피처 차단"
  - 사용자 발화: "애매한 것들은 유심히 지켜볼 수 있는 시스템이 되었네"
  - 별 wave 후보: cps_master 이름 변경 (cascade 영향 큼)
...
- 2026-05-16 wave 완료 — AC 6항목 [x] + §단기 결과 재정의:
  - v0.47.2/v0.47.3 다운스트림 cascade 검증 (harness-upgrade 격하 잔재 자동 삭제)
  - v0.47.5 wiki 자산 wave + v0.47.6 Step 11 false positive·tag-normalize +
    v0.47.7 회고 잔재 자동 정리 = starter 다음 wave 1건 실측 완주
  - **§단기 결과 표를 라인 수 단일 축 → 이전(migration) 효과 5축 증명으로 교체**:
    A 토큰 -34.9%·B 차단 실측 4건·C gate/tag/docs_ops marker 페어링·D 총량
    동결(이전이 본질)·E 다운스트림 마찰 0건
  - 사용자 결론: "scripts 팽창은 비용 아닌 이전의 형상". 다이어트 추가 wave
    불필요 — 게이트 폐기 시 P8 자가 발화 복귀, 모듈 분리만으로는 실효익 없음
