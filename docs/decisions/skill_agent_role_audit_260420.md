...
relates-to:
  - path: harness/implementation_router_redesign_260420.md
    rel: extends
  - path: WIP/harness--info_flow_leak_phase3_measurement_260420.md
    rel: references
status: completed
created: 2026-04-20
updated: 2026-04-20
...
| 대상 | 줄수 | 문제 | 권고 |
|------|------|------|------|
| harness-init/SKILL.md | 383 | L152-155 "Context7 MCP" 언급이 `internal-first.md` 금지 규칙과 충돌 가능 | `external-research-patterns_260420.md` 포인터로 대체 |
...
1. **P0-1 commit + P1-2 review** 쌍으로 먼저 (가장 핫한 핫 패스, 호출 규약
   중복이 최대 리스크) — 별도 WIP
2. **P0-2 eval** — 4관점 named agent 분리 or advisor 대체 선택 후 실행
3. **P0-3 harness-adopt** — "위임 대상 표"만 우선 삽입, 줄 수 감축은 실측 후
4. **P1-1 advisor** — 저비용·저위험, 언제든 끼워넣기
5. **P2** — 실측 누적 후 재평가

각 단계는 본 문서의 결정을 근거로 별도 WIP 생성 → 독립 진행.
...
- 본 문서는 **결정만** 기록. 실행은 각 단계별 WIP가 담당.
