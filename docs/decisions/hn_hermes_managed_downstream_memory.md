...
- 기존 StageLink health cron은 `harness-downstream-guardian` skill 기반 prompt로 갱신하고, weekday에서 **매일 10:00** 실행으로 바꿨다.
...
- executor도 commit/push는 하지 않는다. StageLink 커밋은 downstream의 `/commit` 흐름을 따른다.
...
- [x] StageLink cron prompt 또는 runner가 manifest 기반으로 전환된다.
- [x] StageLink daily guardian cron이 매일 실행되며 read-only로 harness-upgrade 가능성을 보고한다.
...
- [x] Cron report가 memory/reminder를 사실 증거가 아니라 stale 가능 signal로 표시한다.
