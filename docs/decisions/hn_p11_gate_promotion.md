...
### 2026-05-16 — v0.47.11 안전망 보강 (사용자 우려 재검증 후)

사용자 우려로 게이트 재검증 결과 위험 2건 식별 후 즉시 보강:

- **Patch A**: `_DEAD_REF_PATTERNS`의 `staging.md` 단순 basename → `rules/staging.md`
  경로 prefix화. 일반 단어 충돌(다운스트림 `docs/staging.md` 같은 다른 의미)
  회피. 검증: 일반 staging.md 0건 / rules/staging.md 1건 매칭 ✓
- **Patch B**: pre_commit_check.py §3.6 is_starter 분기 추가.
  - is_starter=true → `❌` + ERRORS++ 차단 (starter는 결정적 차단)
  - is_starter=false → `⚠` warn-only, commit 진행 (다운스트림 자율 정비)
  - 이유: starter 폐기 파일명 list가 다운스트림 본문과 우연 충돌 시 작업
    마비 방지. 다운스트림은 alert 받고 자기 일정으로 정비
- test 22 passed (회귀 0), eval --harness 0건 유지

### 2026-05-16 — v0.47.12 AC 헤더 차단 메시지 보강

다운스트림 보고 동반 관찰 1건 (AC 헤더 `##` vs `**bold**` auto-fix 부재,
medium) 흡수. **auto-fix 자동 실행은 추가 안 함** — 빈도(wave당 1회)·학습
곡선·우연 변환 위험 대비 효익 작음.

- pre_commit_check.py L904 차단 메시지에 sed 1줄 추가:
  `auto-fix: sed -i 's/^#\+\s*Acceptance Criteria.*$/**Acceptance Criteria**:/' {wip}`
