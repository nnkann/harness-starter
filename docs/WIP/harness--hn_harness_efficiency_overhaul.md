...
problem: P2
solution-ref: S2 — "review tool call 평균 ≤4회 (micro: 0~2, standard: 1~4, deep: 3~6); maxTurns 소진으로 verdict 누락 0건; docs-only 커밋이 skip 또는 micro로 분류됨"
relates-to:
...
- [ ] Goal: pre_commit_check.py가 1단계 SSOT 따라 frontmatter 검증·검증 묶음 추출·외형 metric 폐기
  검증:
    review: review-deep
    tests: pytest -m "secret or stage"
    실측: 본 WIP commit이 새 검증 묶음 통과 (자기증명)
- [ ] frontmatter `problem`·`solution-ref` 추출 + 누락 시 차단
- [ ] AC `Goal:` + `검증:` 묶음 파싱 + 누락 시 차단
- [ ] CPS 인용 박제 감지 (grep + normalize)
- [ ] 외형 룰 (UPSTREAM_PAT·META_M_PAT·룰 3·4) 폐기
- [ ] `wip_kind`·`has_impact_scope` 추출 폐기, `wip_problem`·`wip_solution_ref`·`ac_review`·`ac_tests`·`ac_actual` 출력
- [ ] eval/SKILL.md — CPS 무결성 감시 추가 (`--harness` 박제 발견)
- [ ] commit 스킬 5.3단계 — tests·실측 자동 실행 (화이트리스트만)
