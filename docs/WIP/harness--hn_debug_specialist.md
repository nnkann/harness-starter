...
relates-to:
  - path: WIP/harness--hn_phase_agent_improvements.md
    rel: caused-by
status: in-progress
created: 2026-04-25
---
...
**Acceptance Criteria**:
- [ ] `.claude/agents/debug-specialist.md` 생성됨
- [ ] frontmatter: name, description(TRIGGER/SKIP 포함), model=sonnet, tools 포함
- [ ] 에이전트 본문: 선행 사례 → 관찰 → 가설 검증 → 보고 4단계 명시
- [ ] "의심 단계에서는 debug-specialist가 먼저" 원칙 명시
- [ ] doc-finder를 첫 번째 행동으로 내재화
...
**Acceptance Criteria**:
- [ ] specialist 풀 표에 `debug-specialist (sonnet)` 행 추가됨
- [ ] 역할·호출 시점 명시됨
