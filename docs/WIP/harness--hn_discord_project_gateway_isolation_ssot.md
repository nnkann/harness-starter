...
s: [S3, S5, S7, S8, S11]
tags: [harness, hermes, discord, gateway, isolation, memory, permissions, ssot]
relates-to:
  - path: docs/WIP/hn_harness_core_overlay_binding.md
    rel: consolidates
  - path: docs/WIP/guides--hn_hermes_integration.md
    rel: consolidates
  - path: docs/WIP/decisions--hn_runtime_adapter_unification.md
    rel: consolidates
  - path: docs/decisions/hn_hermes_managed_downstream_memory.md
    rel: consolidates
  - path: docs/decisions/hn_memory.md
    rel: references
  - path: docs/decisions/hn_reminder_memory_contract.md
    rel: references
status: in-progress
created: 2026-05-29
---
...
- [x] Goal: Discord channel/thread, Hermes gateway dispatcher, project registry, project-bound worker fleet, cwd, memory namespace, permission model을 하나의 SSOT로 묶는다.
  검증:
    review: self
    tests: `rg -n "Project Binding Contract|Memory Boundary|Permission Boundary|Implementation Gate" docs/WIP/harness--hn_discord_project_gateway_isolation_ssot.md`
...
- [ ] Agy review: 본 SSOT가 구현 전제, 누락된 우회 경로, 과잉 설계 여부를 독립 검토한다.
- [ ] Implementation gate: Agy 검토 결과를 반영한 뒤 코드 변경 범위를 확정한다.
...
- gateway route가 없는 channel은 read-only 또는 owner selection 요구.
...
- `docs/WIP/hn_harness_core_overlay_binding.md`: Harness core / downstream
  overlay / local Hermes binding 3계층.
- `docs/WIP/guides--hn_hermes_integration.md`: Hermes는 harness-managed repo를
  감지하고 workflow를 실행하는 orchestration adapter.
- `docs/WIP/decisions--hn_runtime_adapter_unification.md`: 기본 runtime stack은
  Hermes + Codex + Agy.
