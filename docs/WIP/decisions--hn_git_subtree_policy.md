...
- [ ] StageLink에서 실제 subtree 후보(prefix, upstream, update cadence)를 inventory 한다.
- [ ] subtree 후보가 1개 이상 실측되면 하네스 guide 또는 skill에 표준 절차를 추가한다.
- [ ] subtree 정책이 정해지면 `bash-guard.sh`와 `harness-upgrade` 문구가 subtree/worktree를 혼동하지 않도록 보강한다.
