...
### 2. harness-adopt / harness-sync / harness-upgrade 역할 중복 분석

사용자 지적: 세 스킬이 거의 비슷한 것 아닌가?

현재 역할:
- `harness-adopt`: 기존 프로젝트 → 하네스 최초 이식 (1회성)
- `harness-sync`: 클론 후 환경 동기화 (머신당 1회, 의존성·권한·hook 설치)
- `harness-upgrade`: 업스트림 변경 → 다운스트림 반영 (주기적)

중복 가능성 분석 필요:
- adopt와 sync의 경계가 모호한가?
- sync와 upgrade가 겹치는 부분이 있는가?
- 통합 가능한 것이 있는가?
