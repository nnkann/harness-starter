...
작업:
- [ ] 본 세션이 만든 audit WIP 2개 폐기: `docs/WIP/harness--hn_commit_perf_audit.md`, `docs/WIP/harness--hn_commit_perf_followups.md` ✅
  - 처리 방식: `docs_ops.py move` 사용해 status=abandoned + archived/ 이동 OR 단순 git rm (사용자 결정)
- [ ] 본 세션 9b29f23 revert (`git revert --no-edit 9b29f23` 또는 변경 직접 되돌리기)
- [ ] §H-4~H-11 (db402bd, 5/12) 본문 요약 작성 — Phase 6 결정 자료
...
- [ ] `.claude/settings.json` Gemini 자동 호출 hook 제거 (PreToolUse·PostToolUse) ✅
- [ ] `.claude/scripts/orchestrator.py`·`debug-guard.sh`·gemini worker 파일 보존 (수동 호출 가능 상태)
- [ ] `.claude/rules/bug-interrupt.md`의 "## 강제 트리거 (debug-guard.sh)" 절을 "수동 가이드"로 표기 변경 ✅
...
작업:
- [ ] `harness_version_bump.py` patch 트리거 조건 좁힘 — `.claude/scripts/*.{sh,py}` 1줄 수정 = patch 룰 폐기. 사용자 명시 옵트인 또는 다운스트림 영향 명확한 변경만 ✅
- [ ] `pre_commit_check.py` `promotion=release` 자동 트리거를 사용자 명시 게이트로 — commit 스킬이 사용자 확인 후 release 진입 ✅
- [ ] `pre_commit_check.py` `review_route` 강등 조건 확장 — docs-only 외 정량 조건 추가 (단일 small WIP, patch 한정 등) ✅
- [ ] commit/SKILL.md (`.claude/` + `.agents/` 사본) Step 4·5.5·7 본문 갱신 ✅
...
작업:
- [ ] `split-commit.sh` plan 모드에서 `pre_commit_check.py` subprocess 생략 — 호출자가 이미 했을 가능성 처리 ✅
- [ ] split 발동 조건 확장 — 현재 "거대 30+ AND HARNESS_SPLIT_OPT_IN" 좁음. 작은 wave도 char 다양성 있으면 plan 출력
- [ ] 비파괴 default 유지 (`--apply` 옵트인)
...
작업:
- [ ] `pre_commit_check.py` `git diff --cached` 3회 호출 (라인 339·340·341 + 635) → 1회 캐시 ✅
- [ ] `pre_commit_check.py` `HARNESS.json` 중복 read (라인 799·1062) 제거 — 한 번만 ✅
- [ ] `commit_finalize.sh` wip-sync 조건부 실행 — staged에 `docs/WIP/`·`docs/clusters/` 변경 있을 때만 (현재 매 commit) ✅
- [ ] `docs_ops.py wip-sync` abbr 매칭 우선 단축 — 전체 WIP iteration은 abbr hit 0일 때만 fallback ✅
...
작업:
- [ ] `pyproject.toml` 신설 또는 `conftest.py` 사용해 marker 등록: `windows`, `cascade`, `secret`, `gate`, `stage`, `enoent`, `docs_ops`
- [ ] PytestUnknownMarkWarning 0건 확인
...
작업:
- [ ] 모든 변경 1 commit (분리 안 함, 본 wave 자체가 통합)
- [ ] 메시지에 audit #1~#18 해소 매핑·v0.41 baseline 회복·보류 #6 명시
- [ ] v0.46.2 또는 사용자 지정 버전
- [ ] starter push (`HARNESS_DEV=1 git push origin main`)
...
- [ ] Phase 8 완료: 단일 commit + push, audit #1~#18 매핑 명시 + 보류 #6 명시
