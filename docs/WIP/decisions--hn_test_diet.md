...
**Acceptance Criteria**:
- [ ] Goal: commit 흐름 어디에도 무조건 pytest 호출 없음
- [ ] test_pre_commit.py 40개 테스트 모두 영역별 marker 부여
- [ ] SKILL.md(commit)에서 무조건 pytest 절차 제거
- [ ] self-verify.md "테스트 판단" 섹션 — "AC가 요구 시" 트리거로 재작성
- [ ] harness-dev/SKILL.md 체크리스트 동일 변경
- [ ] docs.md AC 포맷에 "pytest -m <marker> 회귀 체크" 표현 명문화
- [ ] 영향 범위: SKILL.md(commit), self-verify.md, harness-dev/SKILL.md — 트리거 변경의 회귀 체크
...
**Acceptance Criteria**:
- [ ] Goal: 다운스트림에 test_pre_commit.py가 복사되지 않음 (신규 설치·upgrade 모두)
- [ ] h-setup.sh에 STARTER_ONLY_FILES 제외 로직 추가
- [ ] harness-upgrade 시 기존 다운스트림의 test_pre_commit.py cleanup 동작 추가
- [ ] starter 전용 파일 컨벤션 문서화 (위치는 작업 중 결정)
- [ ] MIGRATIONS.md 다운스트림 영향 명시 (cleanup 자동/수동 분류)
- [ ] 영향 범위: h-setup.sh — 신규 설치 회귀 체크, harness-upgrade 흐름 회귀 체크
...
**Acceptance Criteria**:
- [ ] decisions/hn_karpathy_principles.md "결정 사항" 섹션에서 "상속 문제 해소" 표현 정정
- [ ] hn_test_diet.md를 후속 작업 포인터로 명시
- [ ] frontmatter status는 completed 유지 (Karpathy 원칙 도입 자체는 완료)
