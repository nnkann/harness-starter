...
**Acceptance Criteria**:
- [ ] Goal: eval --harness가 HARNESS_MAP.md와 실제 파일 간 드리프트 및 관계 그래프 단절을 자동 감지한다 ✅
  검증:
    review: review
    tests: python3 -m pytest .claude/scripts/tests/ -q -k "cps_integrity"
    실측: python3 .claude/scripts/eval_cps_integrity.py 실행 후 "관계 그래프 점검" 섹션 출력 확인
- [ ] HARNESS_MAP.md 존재 여부 체크 (없으면 "미생성" 경고) ✅
- [ ] HARNESS_MAP.md rules 섹션 vs `rules/*.md` 실제 파일 대조 — 누락 시 ⚠ ✅
- [ ] HARNESS_MAP.md skills 섹션 vs `skills/*/SKILL.md` 대조 — 누락 시 ⚠ ✅
- [ ] HARNESS_MAP.md agents 섹션 vs `agents/*.md` 대조 — 누락 시 ⚠ ✅
- [ ] HARNESS_MAP.md scripts 섹션 vs `scripts/*` 대조 — 누락 시 ⚠ ✅
- [ ] "enforced-by 없는 규칙" 감지 — P7 방어 공백 경고
- [ ] "defends-by 없는 Problem" 감지 — 규칙 보호 없는 Problem 경고
- [ ] eval/SKILL.md 보고 형식 예시 갱신 (새 섹션 반영)
...
**Acceptance Criteria**:
- [ ] Goal: `.claude/HARNESS_MAP.md`가 CPS·Rules·Skills·Agents·Scripts·Domains를 양방향 관계로 명시하는 신경망 허브가 된다 ✅
  검증:
    review: review-deep
    tests: python3 .claude/scripts/eval_cps_integrity.py
    실측: eval --harness 실행 후 드리프트 0건 확인
- [ ] 상단에 "전체 Read 필수" 원칙 명시
- [ ] CPS 섹션: P1~P7 + defends-by + served-by 양방향
- [ ] Rules 섹션: Layer 0~3 + defends + enforced-by 양방향
- [ ] Skills 섹션: serves + 위임 대상 + 원본 경로
- [ ] Agents 섹션: serves + 위임 주체 + 원본 경로
- [ ] Scripts 섹션: 자동/수동 분류 + 호출 스킬
- [ ] Domains 섹션: abbr + cluster 경로 + CPS 진입점
- [ ] 역추적 활용 방법 섹션 포함
...
**Acceptance Criteria**:
- [ ] Goal: review 에이전트가 변경 영향 범위의 SKILL.md·rules 미완독을 감지한다
  검증:
    review: review
    tests: 없음
    실측: SKILL.md 변경 커밋 시 review가 Read 흔적 확인하는지 운용 확인
- [ ] "변경 영향 SSOT 문서를 Read tool로 전체 읽었는가" 체크 추가
- [ ] Read 없이 판단한 흔적 발견 시 [경고] 발행
- [ ] HARNESS_MAP.md를 역추적 진입점으로 활용하는 절차 추가 ✅
...
**Acceptance Criteria**:
- [ ] Goal: CLAUDE.md가 HARNESS_MAP.md를 참조해 Claude가 필요 시 신경망 허브를 통과한다 ✅
  검증:
    review: review
    tests: 없음
    실측: 새 세션에서 "P1을 defends하는 규칙들은?" 질문에 HARNESS_MAP.md 참조해 즉시 답변 가능한지 확인
- [ ] CLAUDE.md 적절한 섹션에 HARNESS_MAP.md 참조 링크 + 1줄 역할 설명 ✅
- [ ] "작업 전 HARNESS_MAP.md 통과" 진입 조건 명시 ✅
...
**Acceptance Criteria**:
- [ ] Goal: defends:/serves:/enforced-by: 필드 형식이 공식 정의되어 새 구성요소 추가 시 오매핑 없이 배치 가능하다
  검증:
    review: review
    tests: 없음
    실측: 새 규칙 추가 시 판단 트리 1회 적용 후 HARNESS_MAP.md 배치 확인
- [ ] docs.md에 `defends:`/`serves:`/`enforced-by:` 필드 형식 + 매핑 판단 기준 정의 ✅
- [ ] naming.md L39-40 SSOT 지목이 실제로 충족됨 ✅
- [ ] 새 규칙 추가 판단 트리 명시
...
**Acceptance Criteria**:
- [ ] Goal: harness-upgrade 실행 시 HARNESS_MAP.md가 다운스트림에 자동 전파되고 확인된다 ✅
  검증:
    review: review
    tests: 없음
    실측: harness-upgrade 절차 실행 후 다운스트림에 HARNESS_MAP.md 존재 확인
- [ ] harness-upgrade Step에 "HARNESS_MAP.md 전파 확인" 추가 ✅
- [ ] downstream-readiness.sh에 HARNESS_MAP.md 존재 체크 추가 ✅
