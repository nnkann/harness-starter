...
class ExternalRuntimeDispatcherTests(unittest.TestCase):
    ...
    def test_packet_local_attachment_overrides_only_owner_transport(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            expected_override = ["--provider", transport["provider"], "-m", transport["model"], "-t", "file", "-t", "terminal"]
            ...
            self.assertEqual(dispatched["facts"]["argv"][6:14], expected_override)
            self.assertEqual(popen.call_count, 3)
            for index, call in enumerate(popen.call_args_list):
                argv = call.args[0]
                if index == 0:
                    self.assertEqual(argv[6:14], expected_override)
                else:
    ...
    def test_provider_only_and_model_only_runtime_contracts_reject(self):
        ...
        facts = {
            ...
            "native_profile_ref": "ptah", "native_correlation_id": "d" * 64, "native_runs": [],
