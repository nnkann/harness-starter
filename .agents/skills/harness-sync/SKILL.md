...
- `AGENTS.md`의 `## 환경` 섹션이 채워져 있어야 한다 (패키지 매니저, 빌드/실행 명령어 등).
...
사용자가 강제 재실행을 원하면 마커를 삭제하라고 안내하고, Codex가 임의로 삭제하지 않는다.
...
- 이 스킬은 `AGENTS.md ## 환경`이 소스 오브 트루스라는 전제 위에 작동한다. 파싱 실패 시 harness-init 재실행을 권고.
