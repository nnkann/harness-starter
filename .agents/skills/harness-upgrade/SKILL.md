...
AGENTS.md h-setup.sh docs/harness
...
- `S` (이 파일 skip): 해당 파일 ours 그대로 유지. **`UNAPPLIED_FILES` 목록에 추가** (Step 9.6의 "그 외 미적용" 카테고리에 합류)
...
### Step 9.6. upstream 정합성 자동 검증 (FR-002 대응)
...
**미도달 0건이면**: "✅ upstream 정합성 도달" 한 줄 출력 후 Step 9.7 진행.
...
3. **`docs/harness/migration-log.md` 섹션 자동 생성** (append only, 덮어쓰기 금지):
   ...
   ### 이상 소견
   <!-- 예상 밖 동작, 확인 필요 항목 등 -->
   <!-- Step 9.6 UNAPPLIED_FILES 목록은 본 섹션에 자동 append (1건 이상일 때만) -->
...
5. 완료 보고 (FR-003 대응 — "처리한 횟수"가 아닌 **"upstream 정합성 도달 여부"** 보고):
   ...
   **카운트 데이터 흐름**: Step 9.6에서 채운 `USER_OWNED_FILES`·
