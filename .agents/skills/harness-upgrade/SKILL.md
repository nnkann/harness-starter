...
AGENTS.md h-setup.sh
docs/harness/MIGRATIONS.md docs/harness/MIGRATIONS-archive.md
docs/guides/project_kickoff_sample.md
```
+ **rules 참조 docs (동적 탐색, 아래 참조)**

**`docs/harness/hn_*` 제외** (v0.47.7): 다운스트림 cascade 대상은 "이번
변경 내용"(MIGRATIONS) 한정. starter 내부 회고·결정·incident 히스토리
(`docs/harness/hn_*.md`)는 다운스트림에 가지 않는다. 파일 범위에 명시
2개(`MIGRATIONS.md`·`MIGRATIONS-archive.md`)만 통과.
...
| 3-way merge | `MODIFIED` 중 위 이외 | `AGENTS.md`, `.claude/rules/*`, `.claude/skills/*`, 기타 | 대화형 병합 |
...
- decisions/incidents/WIP/guides: 다운스트림이 작성한 docs. starter는
  `docs/harness/{MIGRATIONS,MIGRATIONS-archive}.md` + `docs/guides/project_kickoff_sample.md`만 관리 (v0.47.7 — hn_* 회고 제외).
...
**(C) starter 회고 cascade 잔재** (v0.47.7 신설) — 옛 버전에서 `docs/harness/*`
통째 cascade로 따라온 starter 내부 회고(`hn_*.md`). 화이트리스트가 좁아진
v0.47.7부터 신규는 차단되나 기존 잔재는 별도 정리 필요. 다운스트림 자체
hn_* 문서는 upstream 부재 판정으로 자동 보존.
...
**(C) starter 회고 cascade 잔재** (v0.47.7 — 다운스트림 전용) — 옛 버전
(v0.47.6 이전)에서 `docs/harness/*` 통째 cascade로 다운스트림에 따라간
starter 내부 회고(`hn_*.md`) 회수. v0.47.7부터 화이트리스트가 MIGRATIONS
2건으로 좁아져 신규 차단은 자동이나, 기존 잔재는 별도 정리 필요.

```bash
LEGACY_HN_FILES=()
if [ "$IS_STARTER" = "false" ]; then
  # docs/harness/hn_*.md 중 upstream에 존재하는 것만 격하 잔재로 판정
  # (다운스트림이 자체 작성한 hn_* docs는 보존 — upstream에 없음)
  while IFS= read -r f; do
    [ -z "$f" ] && continue
    # upstream에 존재하는 파일이면 starter 회고 cascade 잔재
    if git cat-file -e "$UPSTREAM_REMOTE/main:$f" 2>/dev/null; then
      LEGACY_HN_FILES+=("$f")
    fi
  done < <(ls docs/harness/hn_*.md 2>/dev/null)
fi
```

```
🗑️ starter 회고 cascade 잔재 자동 삭제 (N개):
  docs/harness/hn_commit_process_audit.md  (starter 내부 회고 — 다운스트림 미전파)
  docs/harness/hn_advisor_decision_framework.md
  ...
```

```bash
for f in "${LEGACY_HN_FILES[@]}"; do
  rm -f "$f"
  echo "  삭제: $f"
done
```

응답 없이 즉시 삭제. 판정 기준 (`git cat-file -e $UPSTREAM_REMOTE/main:$f`)이
upstream 존재 여부를 결정적으로 검증하므로 다운스트림 자체 작성 hn_* 문서는
**자동 보존**(upstream 부재 → 다운스트림 전용 → skip).
...
| `AGENTS.md` | `## 환경` 섹션 보존. 하네스 공통 섹션만 병합. |
