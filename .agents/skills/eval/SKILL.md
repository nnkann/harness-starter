---
name: eval
description: 하네스·코드 건강 검진. --quick은 30초 헬스체크. --harness는 문서 헬스체크 + CPS 무결성 + 레거시 정비 안내. 주기적으로 돌리는 건강 체크.
serves: S6, S7
---
...
pytest 전체 스위트는 `/eval`의 기본 건강 체크가 아니다. 사용자가 명시하거나
릴리즈/커밋 직전 고위험 공유 코어 변경을 최종 검증할 때만 1회 실행한다.
...
## /eval --harness (문서 헬스체크 + CPS 무결성)

하네스 문서가 **모호·모순·부패**한 부분 + CPS 무결성 + 문서 헬스체크
레이어를 통합한다. 레거시 문서 정비 안내는 부가 출력이다.

문서 생성/수정 품질 게이트의 owner SSOT는 `.claude/rules/docs.md`
"문서 헬스체크 레이어"다.
...
- AGENTS.md와 rules/ 사이 같은 주제를 다른 기준으로 다루는 곳
- rules/ 간 상충
...
CPS 무결성 스캔에서 다음 중 하나라도 해당하면 문서 헬스체크의 레거시 정비
흐름을 안내:
