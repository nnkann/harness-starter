...
> **AGENTS.md `## 환경` drift는 차단 사유 아님** — 다운스트림이 자기
...
| **BIT NEW 플래그** | WIP `## 발견된 스코프 외 이슈`의 `P#: NEW — "..."` 항목을 P# 등록 후보로 자동 인식. 작업 발화와 별개로 각 NEW 항목을 위 매칭 표에 적용해 처리한다. P# 추가는 Codex 단독 판단 가능 (docs.md "CPS 변경 권한" 참조) |
...
어려운 결정, 핵심 설정(AGENTS.md/rules/settings.json) 구조 변경.
...
4. **실행 가능한 AC** — 추상 서술 금지. Codex가 직접 실행하거나 사람이
...
코드 수정은 메인 Codex가 수행. 이 스킬이 하는 일:
...
3. **Phase(작업 단위) 완료 직후 AC 실행 — 필수, 생략 불가**
   ...
   - **자동화 불가 AC** (Codex 행동·UI 동작·운용 효과): "자동 검증 불가 — 운용에서 확인 필요" 명시 의무. 커버되는 범위와 안 되는 범위를 구분해서 제시.
...
- `## 결정 사항`: 결정 + 이유 + 반영 위치 (예: "→ 반영: AGENTS.md ## 환경")
