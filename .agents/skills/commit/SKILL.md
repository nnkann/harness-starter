...
### 4. 하네스 버전 체크 + 연동 갱신 (release path — §H-2, v0.44.1~)
...
§H-2 (v0.44.1~) 이후 pre-check stdout의 **`commit_route` 가 1차 신호**다.
`split_action_recommended`는 보조 신호로만 사용한다 — fast path에서는 분리
권고가 있더라도 사용자가 명시 요청하지 않으면 single로 진행하는 게 기본.
...
커밋 정책은 모두 `.claude/rules/staging.md` 참조. 이 스킬은 §H-2 (v0.44.1~)
이후 pre-check stdout의 **`review_route` 를 1차 신호**로 받아 review를
호출한다. 옛 `recommended_stage`는 호환성 폴백 — `review_route`가 비었을
때만 사용 (정상 흐름에선 비지 않음).
