...
1. 기존 프로젝트 현황 스캔 (`.claude/`·`docs/`·`AGENTS.md` 탐지)
...
| 영역 | 위임 |
|------|------|
| 문서 이동·clusters 갱신 실행 | `.claude/scripts/docs_ops.py` |
...
| Pass (나→`docs_ops.py`) | `cluster-update` / `validate` / `verify-relates` 서브커맨드 |
| Pass (나→harness-init) | 기존 스택 추출 결과 · 기존 도메인 목록 · 기존 AGENTS.md 스냅샷 |
| Preserve | 기존 파일 원본 백업 경로 · 이식 전 상태 스냅샷 · 사용자 승인받은 도메인 목록(추론값 그대로 확정 금지) · 기존 README/AGENTS.md 본문 |
| Signal risk | ⛔ README/AGENTS.md 덮어쓰기·기존 `.claude/` 파손 위험 · ⚠️ 도메인 추론 자신감 낮음·3-way 병합 충돌 다수 · 🔍 단계별 사용자 승인 이력 |
...
- 기존 프로젝트에 `.claude/`, `docs/`, `AGENTS.md` 중 하나 이상이 이미 존재할 수 있다.
...
| Step 2. AGENTS.md 병합 | **필수** | 하네스 규칙의 진입점 |
...
**기존 AGENTS.md가 없으면**: 하네스 템플릿을 그대로 복사한다.
...
- **날짜 접미사(`_YYMMDD`)는 제거를 제안**한다 (naming.md v0.16.0 "날짜 suffix
  전면 금지"). 발생 시점은 프론트매터 `created` + git history가 담당.
...
```bash
bash .claude/scripts/docs_ops.py cluster-update
```

docs_ops.py가 naming.md 약어 표 + docs/ 하위 md를 전수 스캔해
...
### 다음 할 일
  ...
  2. `/eval --harness` 실행 (권장) — 하네스 문서 품질 점검 + 레거시 정비 안내
     (doc-health 흡수). abbr rename·CPS frontmatter 추가 가이드 제공.
...
| 검증 항목 | 확인 방법 |
|----------|----------|
| AGENTS.md에 하네스 절대 규칙 존재 | `## 절대 규칙` 섹션 + `<important>` 블록 확인 |
