...
#
# 프로파일:
#   minimal  (기본) — 필수 스킬만 줄인 기능 minimal. runtime adapter 표면은 동일하게 설치.
...
if [ ! -f "$KICKOFF" ] && [ ! -f "$TARGET/docs/guides/project_kickoff.md" ]; then
  KICKOFF_CREATED=$(date +%Y-%m-%d)
  {
    printf '%s\n' '---'
    printf '%s\n' 'title: 하네스 초기화 대기 중'
    printf '%s\n' 'domain: harness'
    printf '%s\n' 'c: "h-setup.sh가 하네스 파일을 설치했지만 harness-init이 아직 프로젝트 CPS와 환경 결정을 만들지 않았다"'
    printf '%s\n' 'problem: [P7, P9]'
    printf '%s\n' 's: [S7, S9]'
    printf '%s\n' 'tags: [init, setup, downstream]'
    printf '%s\n' 'status: pending'
    printf '%s\n' "created: $KICKOFF_CREATED"
    printf '%s\n' '---'
    cat <<'EOF'
# 하네스 초기화 대기 중
