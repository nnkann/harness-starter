...
현재 버전: **v0.30.2** — 0.x = 공개 API·동작 불안정·실험 단계. 다운스트림 실측 누적·매처 동작 검증·README 격차 안정화 후 1.0.0 검토. 변경 이력은 `git log --oneline --grep "(v0\."`, 다운스트림 마이그레이션은 `docs/harness/MIGRATIONS.md`.
...
### v0.30.2 (2026-05-02) — MIGRATIONS·README 슬림화 + archive 자동화 (patch)

MIGRATIONS.md 759줄 → 240줄 (24개 → 5개), README 420줄 → 297줄. 6번째
이전 섹션은 `MIGRATIONS-archive.md`로 이동. `harness_version_bump.py
--archive` 서브커맨드 신설 (멱등성). commit Step 4가 매번 자동 호출 →
본문 비대화 방지.

### v0.30.1 (2026-05-02) — wip-sync 매칭 정밀화 + 위임 트리거 강화 (patch)

`docs_ops.py wip-sync` 부분 매칭 false positive 차단 — 정규식을 체크박스
라인 한정으로 좁히고 frontmatter 영역 마킹 제외. `session-start.sh`
연속 fix 감지를 prefix 무관 "공통 파일 2 커밋 연속 수정"으로 확장
(메타 파일 노이즈 제외). `no-speculation.md`에 "동일 시스템 동작 이슈
2회 이상" 트리거 명시. v0.29.2·v0.30.0 자기증명 사고 대응.

### v0.30.0 (2026-05-02) — eval --harness CPS 무결성 감시 (minor)

`.claude/scripts/eval_cps_integrity.py` 신규 — docs/ 전수 frontmatter
solution-ref 박제 grep + Problem 인플레이션·인용 빈도 측정.
`pre_commit_check.py`의 헬퍼 동적 import 재사용 (코드 중복 0).
`eval/SKILL.md` `--harness` 점검 항목 5(CPS 무결성) 통합.

### v0.29.2 (2026-05-02) — commit 5.3 자동 실행 코드 구체화 (patch)

`commit/SKILL.md` Step 5.3 — pre-check 출력 `ac_tests`·`ac_actual`을
화이트리스트(`pytest|bash -n|python -m|grep`) 기반 자동 실행. 실패 시
commit 차단. 작성자 자가 보고 시스템과 정합.

### v0.29.1 (2026-05-02) — Phase 2-A 2단계: AC + CPS 시스템 강제 (patch)

frontmatter `problem`·`solution-ref` 검증 + AC `Goal:` + 검증 묶음
추출 + CPS 박제 감지. 외형 metric 룰 (kind 라벨·줄 수·경로) 일괄
폐기. `wip_problem`·`wip_solution_ref`·`ac_review`·`ac_tests`·
`ac_actual` 출력으로 commit 흐름 자동화.
