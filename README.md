...
현재 버전: **v0.43.3** — 0.x = 공개 API·동작 불안정·실험 단계. 다운스트림 실측 누적·매처 동작 검증·README 격차 안정화 후 1.0.0 검토. 변경 이력은 `git log --oneline --grep "(v0\."`, 다운스트림 마이그레이션은 `docs/harness/MIGRATIONS.md`.
...
### v0.43.3 (2026-05-11) — Codex 하네스 브리지와 Gemini 위임 WIP 정렬 (patch)

Codex 환경에서 동일 하네스 지시가 로드되도록 루트 `AGENTS.md`, `.agents/skills/**`, `.codex/agents/*.toml`, `.codex/hooks.json` 브리지 추가. `pre_commit_check.py`와 `orchestrator.py`의 하네스 자가 검증 경로를 보강하고, Gemini delegation pipeline 결정 문서를 WIP로 되돌려 Phase 후속 작업이 진행 중임을 명시. 세션 거짓 완료·자기 위반 패턴 incident WIP 추가.

### v0.43.2 (2026-05-11) — gemini_delegation_pipeline Phase 1 (CPS Solution 자동 Gemini 의견) (patch)

orchestrator.py에 CPS Solution 변경 staged 시 gemini CLI 자동 호출 트리거 추가. `detect_solution_change()` + `gemini_cli_available()` (graceful skip — 미설치 다운스트림 영향 0) + `call_gemini_background()` (Popen detach, hook 비차단) + 세션당 1회 호출. 결과 `.claude/memory/gemini-solution-review.md`. gemini_delegation_pipeline 결정 Phase 1만 박제 (Phase 2 = orchestrator MVI 완료, Phase 3 = 별 wave). 회귀 가드 3건 신설 → 10/10 통과. WIP completed 전환.

### v0.43.1 (2026-05-11) — orchestrator P1 신호 stale 누적 해소 (upsert) (patch)

v0.43.0 직후 실측 — orchestrator.py 자기 수정·README 수정마다 P1 INFO 신호가 count 변화별로 별도 누적되어 PreToolUse 컨텍스트에 stale 신호 3건+ 동시 출력. 원인: `deduplicate_signals` (p_id, message) 키 사용 — count 변화 시 message 달라져 새 식별자. 해소: 신호에 `key` 필드 (`P1:{file_path}`) — count 무관 안정 식별자. `_signal_key()` 헬퍼 + `deduplicate_signals` 재작성으로 upsert 동작. `key` 없는 정적 신호는 기존 (p_id, message) fallback 호환. 회귀 가드 2건 신설 → 7/7 통과. review 경고 1 (harness-init false-positive)은 시나리오 오인 — 다운스트림은 init 실행 안 함 (업스트림 CPS 그대로 전파) → revert.

### v0.43.0 (2026-05-11) — 오케스트레이터 MVI 도입 (PreToolUse hook + P9 강제 cascade) (minor)

P9 (정보 오염의 관성)·S9 (주관 격리 + 다층 검증) 결정 시리즈의 실측 구현. `.claude/scripts/orchestrator.py` 신설 (~290줄) + PreToolUse hook 등록으로 LLM 자가 발화 의존을 커널 강제로 전환. 매 도구 호출 직전 객관 신호 detect — P1 (동일 파일 연속 수정 카운터)·P9 (WIP frontmatter `problem` ↔ CPS Problems 매칭). 이중 안전장치: stdout `additionalContext` JSON + `.claude/session_signal.json` 파일 쓰기 (Issue #13912 대비). P9 cascade 깨짐 시 **Exit 2 강제 중단** — Praetorian 8계층 모델 + arXiv:2503.13657 "Ignored other agent's input" 실패 모드 차단. 동시에 Layer 2 도구 frontmatter `trigger:` 필드 스키마 정의 (docs.md). 3개 결정 시리즈 (cps_entry_signal_layering / bit_cascade_objectification / orchestrator_mechanism) 메타 cascade 완성. researcher 외부 자료 + Gemini 2차 외부 시각 위임 반영. 5/5 회귀 통과.

### v0.42.0 (2026-05-10) — eval --harness CLI 백엔드 + 검증 도구 정렬 진단 (minor)

eval --harness가 LLM 해석 의존이던 결정적 측정 항목을 CLI 백엔드(`eval_harness.py`)로 이전. 단일 진입점에서 항목 5(CPS 무결성)·6(방어 활성)·7(피드백 리포트)·8(검증 도구 정렬 신규) 결정적 실행. SKILL.md 본문은 LLM 해석 영역(항목 1~4)만 담당. 신규 항목 8은 TS/JS 프로젝트의 LSP/lint/tsc가 산출물(dist) 아닌 src 직접 보도록 보장 — 4신호(워크스페이스·codegen·dist 자체 소비·outDir 분리) 검출 후 정렬 측정. 외부 명령(npm·tsc) 호출 0건. 직전 v0.41.0 wip_util_ssot 후속 의무 박제 — eval_harness.py가 wip_util import. test_eval_harness.py 7건 신설, 전체 82 passed.

### v0.41.0 (2026-05-10) — WIP 파싱 SSOT 통합 (wip_util.py + post-compact-guard.py 전환) (minor)

WIP frontmatter 파싱이 3곳에 파편화돼 있던 상태(session-start.py·post-compact-guard.sh·stop-guard.py)를 단일 SSOT(`utils/wip_util.py`)로 통합. stop-guard 자기복제 케이스가 다른 sh에 적용되는지 14개 sh 점검 결과, 적합 1건(post-compact-guard.sh) + 부적합 12건. 사용자 통찰("언어 전환이 아닌 로직 통합")로 SSOT 부재가 진짜 원인이라는 결론. `wip_util.py` 신설 + session-start.py·stop-guard.py가 import 사용으로 마이그레이션 + post-compact-guard.sh를 Python으로 전환(sed/grep/awk 혼재 제거). settings.json PostCompact command `bash` → `python3` 갱신, sh 삭제 (dead code 동시 제거). 다운스트림은 settings.json 자체 커스터마이즈한 경우 3-way merge 후 PostCompact command 확인 필요.
