...
현재 버전: **v0.34.1** — 0.x = 공개 API·동작 불안정·실험 단계. 다운스트림 실측 누적·매처 동작 검증·README 격차 안정화 후 1.0.0 검토. 변경 이력은 `git log --oneline --grep "(v0\."`, 다운스트림 마이그레이션은 `docs/harness/MIGRATIONS.md`.
...
### v0.34.1 (2026-05-02) — amplification 후속 4 wave 처리 (cluster 게이팅·WIP 가시성·glob 가이드·adopt 안내) (patch)

amplification baseline (Phase 4-A)에서 식별된 5 병목 중 (a) v0.34.0 외
나머지 4개 처리: (c) `cluster-update` 결정적 출력 + diff 비교로 동일 본문
시 write skip (mtime noise 0). (b) cluster 본문에 `## 진행 중 (WIP)` 섹션
자동 등록으로 사용자·에이전트가 cluster scan 한 번에 completed + 진행 중
WIP 발견. (d) `docs.md`·`naming.md` 가이드에 양쪽 wildcard
(`docs/**/*<abbr>_*`) 명시 + cluster 진입점 격상 (라우팅 태그 폐기 안 함).
(e) `harness-adopt` Step 8 "다음 할 일"에 `/harness-init` 미실행 시
implementation 차단됨을 사전 강조 — (a) 사후 차단과 이중 안전망.
`TestClusterUpdateGating` 3 회귀 추가, pytest -m docs_ops 25/25 통과.
다운스트림 양식·인터페이스 무변경, 자동 이행.

### v0.34.0 (2026-05-02) — implementation init 게이트 의미 재정의 (A4) (minor)

3환경 baseline 측정에서 v0.33.x까지 init 게이트가 다운스트림 양식 drift
환경(C++/CMake처럼 `패키지 매니저:` 키 N/A)에서 false-block + 15~19s 헛돔
입증. advisor 4 대안 weighted matrix 평가 후 A4 채택 (96점):
"drift 감지"→"init 안 돈 신규 프로젝트만 차단"으로 의미 재정의.
`docs/guides/project_kickoff.md` 부재 OR `status: sample` 단독 → 차단.
`check_init_done.sh` 추출, TestInitGate 5 회귀 통과. starter 실측 ~0.07s
(게이트 ≤2s 27x 여유). 다운스트림 자유도 회복.

### v0.33.1 (2026-05-02) — SEALED 면제 (MIGRATIONS류 자기 운영 파일) (patch)

다운스트림 incident: v0.32.0 SEALED 룰 도입 시 starter 자기 운영 누적
파일(MIGRATIONS.md / MIGRATIONS-archive.md / migration-log.md) 면제 누락.
다운스트림이 `harness-upgrade`로 v0.33.0 fetch 후 `/commit` 시
MIGRATIONS.md가 차단됐다는 보고 수령. debug-specialist 진단 후
`SEALED_PATH_EXEMPT` path 화이트리스트 추가, T42.5 회귀 테스트.
pytest 58 passed (기존 57 + 신규 1). 다운스트림 정상 흐름 복귀.

### v0.33.0 (2026-05-02) — commit_finalize wrapper (wip-sync + git commit 단일 흐름) (minor)

자기증명 사고: SKILL.md SSOT가 "git commit 직전 wip-sync"를 명시했지만
Claude가 git commit 먼저 호출 → wip-sync → 별 이동 commit 패턴 반복
(8 commit 중 3건 위반, 37.5%). 자율 신뢰 부족 → 메커니즘 차단으로 전환.
`.claude/scripts/commit_finalize.sh` wrapper 신설 — VERDICT != block일 때
wip-sync 자동 실행 → 이동·cluster·역참조 갱신 staging → git commit 단일
호출. 1 wave = 1 commit. TestCommitFinalize 3/3 통과. 본 commit 자기증명.

### v0.32.0 (2026-05-02) — 약속 박제 보호 (completed 봉인 + anti-defer 룰) (minor)

자기증명 사고 대응. v0.31.2 commit 후 completed wave WIP를 같은 세션에서
무단 확장 시도 → 시스템 신뢰성 파괴 패턴 인식. 세 가지 메커니즘:
(1) pre_commit_check.py에 completed 봉인 게이트 신설 — status: completed
문서 본문 무단 변경 시 exit 2 차단, `docs_ops.py reopen` 절차 의무.
(2) `.claude/rules/anti-defer.md` 신설 — "측정 후·다음 세션·데이터 누적
필요" 같은 미루기 회피 사유 단독 사용 금지. 별 wave 분리는 정상이지만
처리 시점이 "후속"이면 미루기로 간주. (3) review.md 검증 루프 7번에
wave scope 무단 확장 감지 추가. TestCompletedSeal 5/5 통과. 본 commit
자기증명 — 봉인 게이트가 본 작업 자체를 차단해 reopen 절차 거치게 함.
