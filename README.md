...
현재 버전: **v0.51.5** — 0.x = 공개 API·동작 불안정·실험 단계. 다운스트림 실측 누적·매처 동작 검증·README 격차 안정화 후 1.0.0 검토. 변경 이력은 `git log --oneline --grep "(v0\."`, 다운스트림 마이그레이션은 `docs/harness/MIGRATIONS.md`.
...
### v0.51.5 (2026-05-19) — CPS P# 정련 + memory/reminder 회귀 계약 보강 (patch)

P12~P15 대량 신설안 폐기, 신규 P# 0개 원칙 유지. **P6/P7/P8/P9/P11** 경계를 보강하고 `C → P → S → AC` 연결 계약을 명시. **S9**는 P9 primary 작업의 회귀 가드 기본값(회귀 테스트 또는 재오염 방지 실측)을 포함. `.claude/rules/memory.md`는 memory/signal을 판단 원자료가 아닌 환기 신호로 재정의하고, 회귀 signal 3단계(환기→재확인→검증 선택)를 추가. `session-start.py`·`stop-guard.py`는 memory count 단독 출력과 상시 reminder 노이즈를 축소. `docs_ops.py move`는 `cps--` WIP의 `docs/cps/` 승격을 지원. pytest 효율/회귀 라우팅은 별도 WIP로 분리.

### v0.51.4 (2026-05-18) — P12·S12 폐기 + P11에 흡수 (patch)

P12 박제 직후 LLM이 정확히 P12 패턴 위반 실측 (별 WIP 4개 자기 분리). codex·gemini 재검토 합의로 **P12·S12 폐기**, P11에 "sub-task 분리 우회 금지" 흡수. `project_kickoff.md` P12/S12 표·본문 폐기 + P11/S11 본문 보강. `.claude/rules/code-ssot.md` "sub-task 분리 우회 금지" 1단락 추가. `docs/decisions/hn_split_completion_bypass.md` → `docs/archived/`. `pre_commit_check.py` §3.7 게이트 정밀화 (기존 P# 행 갱신 vs 신규 신설 구분, 회귀 테스트 +1). `cp_split_completion_p12.md` 사례 잔류 (P11 첫 회피). 결정: `docs/decisions/hn_claude_dir_audit.md`.

### v0.51.3 (2026-05-18) — .claude/ 잔재 정리 + skills/agents serves: 다중 매핑 (patch)

P11/P12 자기 적용. codebase-analyst + codex + gemini 3개 의견 종합. **skills 13개 + agents 9개** frontmatter `serves: S#` 다중 매핑 정착 (`advisor` S5→S1·S8, `commit` S6·S9, `eval` S6·S7, `harness-init` S6·S9 등). **MEMORY.md** 인덱스 6개 누락 등록 + signal_* 섹션 분리. **`.claude/rules/memory.md`** "누적 감사 로그" 섹션 신설 + `stop_hook_audit.log` 박제. 별 wave 후보 4건 WIP 분리 (guard 점검·보안 P# 신설·signal 스키마·Solutions dead link) — P12 위반 회피. 결정: `docs/decisions/hn_claude_dir_audit.md`.

### v0.51.2 (2026-05-18) — CPS P# 신설 cp_{slug}.md 박제 누락 차단 + cps_add 표 갱신 (patch)

P12 박제 wave에서 노출된 동형 박제 위치 분산 결함 (P11 직격) 차단. `pre_commit_check.py` §3.7 신설 — staged diff에 `project_kickoff.md` `| P\d+ |` 신규 행 + `docs/cps/cp_*.md` 신규 파일 동반 staging 없으면 차단. `docs_ops.py cmd_cps_add` Problems 표 마지막 P# 행 뒤에 새 P# 행 자동 삽입 (기존엔 `### P# — ...` 헤더만 append, 표 누락 잠복). 회귀 테스트 4건. 결정: `docs/decisions/hn_commit_skill_completeness.md` (예정).

### v0.51.1 (2026-05-17) — cascade_docs SSOT 회수 + archived target 검사 확장 (patch)

v0.51.0 직후 review 관찰 + Step 0 감사 잔여 회수. **SSOT 회수**: `cascade_docs.py`의 `_resolve_relates_path` local copy 제거 → `docs_ops._resolve_relates_path` 동적 import로 SSOT 단일 정의 보존(본문 복제 금지 원칙). 절대경로 호환은 wrapper 1줄. **archived target 검사 확장**: `_is_decisions_target`에 `docs/archived/` 추가 — v0.47.7 cascade 정책상 archived도 미전파라 같은 dead link 메커니즘. **cleanup**: `hn_harness_73pct_cut.md`의 `extends archived/...` 제거 + 본문 §0에 baseline 확장 정보 1줄 보전. 회귀 테스트 7번째 케이스 추가. starter `cascade_docs check` 위반 0건 정합.

### v0.51.0 (2026-05-17) — cascade boundary projection (FR-002 방어) (minor)

다운스트림 보고 FR-002 처리. starter `docs/decisions/hn_*.md` 부분 cascade 정책(v0.47.7)에서 cascade되는 decision이 `relates-to`로 비-cascade 동료를 가리키면 다운스트림에 영구 dead link 발생. **3엔진(advisor·Gemini·Codex) 비교 후 옵션 1+3 결합 채택** — 옵션 2(신규 rel type) 만장일치 탈락(taxonomy 오염). **`.claude/scripts/cascade_docs.py` 신설** — projection-safe helper. `compute_cascade_set()`(rules·skills·agents 본문 grep), `strip_non_cascading_relates()`(frontmatter 정제), `check_cascade_boundary_violations()`(전수 감사). CLI: `check`/`rewrite`. **`docs_ops.py cmd_verify_relates` 확장**: starter 환경에서 cascade boundary 위반 자동 차단(작성 시점), 다운스트림은 warn-only 그대로. **`harness-upgrade` Step 3 호출 추가**: 다운스트림 복사 전 frontmatter projection-safe 정제(upstream 원본 불변, projection drift는 의미 drift 아님). **starter 기존 2건 cleanup**: `hn_code_ssot_rule.md`(caused-by 제거, 본문 인용 유지), `hn_harness_73pct_cut.md`(supersedes 제거, §0 본문 1줄 보전). 회귀 테스트 6/6 통과. Step 0 감사에서 이미 2건 잠복 사례 발견 → 옵션 3 필요성 증명. 결정: `docs/decisions/hn_decisions_cascade_partial_graph.md`.
