...
현재 버전: **v0.34.7** — 0.x = 공개 API·동작 불안정·실험 단계. 다운스트림 실측 누적·매처 동작 검증·README 격차 안정화 후 1.0.0 검토. 변경 이력은 `git log --oneline --grep "(v0\."`, 다운스트림 마이그레이션은 `docs/harness/MIGRATIONS.md`.
...
### v0.34.7 (2026-05-04) — eval_cps_integrity 다운스트림 호환성 강화 + completed 봉인 frontmatter 면제 (patch)

`eval --harness`의 "Solution 충족 인용 분포" 섹션이 `**S1**` 굵은 글씨 CPS 형식에서
빈 결과 출력되는 버그 수정. `docs/harness/` 폴더를 스캔 제외해 upstream CPS 참조
문서 오탐 방지. `reopen → solution-ref 수정 → move` 정상 절차 후에도 completed 봉인이
차단하는 문제 해소 — frontmatter 블록 내 변경을 면제 조건에 추가. 테스트 71/71 통과.

### v0.34.5 (2026-05-03) — supabase/migrations/*.sql PostgreSQL role 이름 오탐 면제 (patch)

`GRANT/REVOKE ... TO/FROM service_role` 등 PostgreSQL DCL의 role 이름이
`S1_LINE_EXEMPT` 누락으로 line-confirmed 차단됐음. `supabase/migrations/*.sql`
면제 추가 + `install-secret-scan-hook.sh` EXEMPT_RE 동기화. `pytest -m secret` 5/5.

### v0.34.4 (2026-05-03) — pre-check false-block 2건 수정: AC 에러 메시지 개선·service_role 환경변수 이름 면제 (patch)

AC 섹션 헤더 형식 사용 시 에러 메시지에 bold 형식 힌트 추가. `service_role`
패턴에 `(?![A-Z_])` negative lookahead 추가 — `process.env.SUPABASE_SERVICE_ROLE_KEY`
같은 환경변수 이름은 면제, 값 리터럴은 계속 차단.

### v0.34.3 (2026-05-03) — completed 봉인 relates-to path 수정 면제 (dead-link 복구 루프 해소) (patch)

v0.34.2 verify-relates 전수 검사와 completed 봉인이 충돌해 영구 차단 루프
발생 — dead relates-to 수정 시 봉인 차단, 봉인 상태로 수정 불가. 봉인
면제 화이트리스트에 `- path: <경로>` 라인 변경 추가. dead-link 복구(경로
수정·항목 제거)가 즉시 가능해짐. T42.6 회귀 추가, pytest -m gate 18/18 통과.

### v0.34.2 (2026-05-02) — verify-relates pre-check 통합 (커밋 시 relates-to 전수 검사) (patch)

다운스트림 v0.34.1 검증에서 relates-to dead link 7건 발견, debug-specialist
진단으로 H3(박제 ref) 확정. pre-check 3.5단계 섹션 C를 staged 파일 단독
검사 → `cmd_verify_relates` 전수 호출로 교체. 기존 커밋된 파일의 깨진 ref도
커밋 시 즉시 차단. 비용 0.13s(N=18 환경 실측). 사전 부채 있는 다운스트림은
upgrade 후 `verify-relates` 실행·수정 필수(수동 적용).
T45.1·T45.2 회귀 추가, pytest -m docs_ops 27/27 통과.
