...
### v0.44.1 (2026-05-13) — 커밋 파이프라인 route 출력·side effect ledger·Windows smoke (patch)

`hn_commit_perf_optimization` wave 통과. `pre_commit_check.py` stdout에 8개 route 키 (`commit_route`·`review_route`·`promotion`·`blocking_reasons`·`warning_reasons`·`side_effects.{required,release,repair}`) 도입. `review_route`는 객관 신호 4종 만족 시 `micro`로 정량 강등. `get_secret_patterns()` SSOT 함수 + `--emit-secret-patterns` 서브커맨드. `warning_reasons`에 cascade integrity 신호 누적. `docs_ops.py wip-sync`·`commit_finalize.sh`에 side effect ledger 4종 출력. `split-commit.sh` 비파괴 planner 전환 (`--apply` 명시 시에만 destructive). `bash-guard.sh` CRLF 156개 LF 정규화. `commit/SKILL.md` Pass 표·Step 4/5.5/7·최종 요약 재작성. 회귀 가드 9건 신설 (`pytest -m "stage or windows or cascade"` 16 passed).
