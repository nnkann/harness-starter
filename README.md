...
현재 버전: **v0.13.3** — 0.x = 공개 API·동작 불안정·실험 단계. 다운스트림 실측 누적·매처 동작 검증·README 격차 안정화 후 1.0.0 검토. 변경 이력은 `docs/harness/promotion-log.md`, 다운스트림 마이그레이션은 `docs/harness/MIGRATIONS.md`.
...
└── scripts/                     hook 스크립트 + 회귀 테스트 (10개)
    ...
    ├── test-pre-commit.sh       회귀 테스트 (30 케이스)
...
업그레이드 후 검증:
  bash .claude/scripts/test-pre-commit.sh    # 30/30 기대
