...
현재 버전: **v0.18.2** — 0.x = 공개 API·동작 불안정·실험 단계. 다운스트림 실측 누적·매처 동작 검증·README 격차 안정화 후 1.0.0 검토. 변경 이력은 `docs/harness/promotion-log.md`, 다운스트림 마이그레이션은 `docs/harness/MIGRATIONS.md`.
...
└── scripts/                     hook 스크립트 + 회귀 테스트 (12개)
    ...
    ├── test-pre-commit.sh       회귀 테스트 (45 케이스, 5줄 룰 T21~T32 포함)
...
업그레이드 후 검증:
  bash .claude/scripts/test-pre-commit.sh    # 45/45 기대
...
### v0.18.2 (2026-04-22) — T13 재진단 훅 (원인 미확정)

v0.18.1 fix(unique 파일명) 적용 후에도 다운스트림에서 T13.1 exit 2 지속 → 최초 가설(git log 교차)이 원인이 아님 자인. `TEST_DEBUG=1`로 FAIL 시 캡처된 pre-check stdout+stderr를 dump해 실제 원인 관찰 가능. incident 문서 정정(status: in-progress). 운영 로직은 무변경.

### v0.18.1 (2026-04-22) — T13 테스트 격리 fix (부분)

테스트 케이스(T13) 파일명을 PID + 에포크로 unique화. 고정 경로 교차 가능성 봉쇄. 다운스트림 실제 실패의 전면 해결은 아님 — v0.18.2 재진단 훅 참조.
