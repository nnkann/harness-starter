...
현재 버전: **v0.44.1** — 0.x = 공개 API·동작 불안정·실험 단계. 다운스트림 실측 누적·매처 동작 검증·README 격차 안정화 후 1.0.0 검토. 변경 이력은 `git log --oneline --grep "(v0\."`, 다운스트림 마이그레이션은 `docs/harness/MIGRATIONS.md`.
...
### v0.44.1 (2026-05-12) — pre_commit_check route 출력 추가 (patch)

`harness--hn_commit_perf_optimization.md` §H-1 좁힘 후 첫 닫힘. `pre_commit_check.py` stdout 끝에 6키 추가 — `commit_route` (single|sub), `review_route` (skip|micro|standard|deep), `promotion` (none|release|repair), `side_effects.required/release/repair`. `is_starter=true` + `.claude/HARNESS.json`·`docs/harness/MIGRATIONS.md`·`README.md` staged 시 `promotion=release` 발화. SKILL은 손대지 않음 — 다음 wave (§H-2 SKILL이 route 소비)의 입력 계약 freeze. `TestRouteOutput` 3건 추가 (기본 케이스·시크릿 차단·release 후보) + 기존 13개 키 회귀 가드 (TestStageBasic 2건 통과). 별 WIP `harness--hn_commit_perf_followups.md` 신설 (§H-2~7 인덱스).
