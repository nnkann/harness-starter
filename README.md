...
현재 버전: **v0.42.1** — 0.x = 공개 API·동작 불안정·실험 단계. 다운스트림 실측 누적·매처 동작 검증·README 격차 안정화 후 1.0.0 검토. 변경 이력은 `git log --oneline --grep "(v0\."`, 다운스트림 마이그레이션은 `docs/harness/MIGRATIONS.md`.
...
### v0.40.1 (2026-05-10) — stop-guard.sh → stop-guard.py 전환 (자기증식 차단) (patch)

v0.40.0 stop-guard.sh 도입 직후 검증에서 `grep -c || echo 0` Git Bash 호환 결함 발견 + 1회 fix. 이 1회 fix가 자기증식 신호(bash 파싱 3종 혼재 + 추측성 방어 코드 누적). pre-emptive Python 전환 — bash 4개 동작 절 1:1 포팅. session-start.py와 frontmatter 파싱 일관성 + Windows cp949 안전 처리 답습. settings.json hook command 1줄 변경 (Reversibility 5/5). audit log 형식 호환 (기존 누적 로그 무수정). 사용자 통찰("자가증식하는 거 같은데?") + anti-defer "별 wave 신설 + 즉시 처리" 답습 — 모체 wave 커밋 직후 발견은 운용 후 발견 아니므로 즉시 처리가 정합.

### v0.40.0 (2026-05-10) — P8 Phase 3: incident 회상 + signal lifecycle + stop-guard 조건 C (minor)

P8 자가 의존 보강 3축 1차 도입 (advisor + 사용자 라운드 합의). **D-Lite**: `session-start.py` `section_incidents()` 신설 — 현재 WIP `domain` ∩ `docs/incidents/*.md` `domain`, 30일 + 최대 3건 자동 출력 (tags ∩ symptom-keywords 매칭은 Phase 4 유보). **E**: signal lifecycle 변경 — incidents 승급 시 signal 삭제 → `archived: true` 마커 잔존, session-start.py가 약한 톤 출력 (회상 다리 유지). **B**: `stop-guard.sh` 조건 A·B·C AND 발화 (Soft + Dry-run) — git 수정 + WIP in-progress + 빈 체크박스/BIT 블록 부재 hit 시 stderr 1줄 + `.claude/memory/stop_hook_audit.log` append (gitignore). 차단 아님 — 측정용. C(AC 자동 마킹)·A(PostToolUse hook) 보류. Hard Stop은 운용 5~10 commit 후 Phase 4 결정. signal 4건 신설 — `hn_commit_process_gaps` 5건 패턴 중 무신호 4건 보강.
