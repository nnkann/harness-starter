...
| (하네스 고유) | — | 테스트 형식 과잉 | 속도·유지비 | Task 7 진단 → Phase 2 제거 ✅ |
...
**단계 전략 (모두 완료 ✅)**:
- Phase 1: 원칙 추가 + SSOT 충돌 해소 + AC 연결 기반 구축 ✅
- Phase 2: 테스트 진단 결과 기반으로 형식적 테스트 제거 ✅
- Phase 3: docs.md 배경 텍스트 다이어트, review 구조 개선 ✅
...
### 8. Task 6 보강 — task 단위 AC scope + review diff 조건 명시
> kind: bug
...
- **pre_commit_check.py + staging 신호 체계 폐기 방향 결정**: 파일 경로 기반 신호 판정(S1~S15) + stage 결정 로직은 AC가 없던 시절 추측으로 검증 범위를 결정하던 구조. WIP AC의 `kind:` + `영향 범위:`가 이미 검증 범위를 선언하므로 이 레이어 전체가 불필요. → Task 6에서 실제 제거 작업 ✅
- **거짓 ✅ 정정 (2026-04-30)**: 위 항목에 "다운스트림 56개 테스트 상속 문제 해소" 표현이 있었으나 실제로는 신호 테스트만 제거됐고 다운스트림 상속·매 커밋 무조건 실행 부담은 미해결이었음. 후속 작업: `docs/WIP/decisions--hn_test_diet.md` (테스트 다이어트 + h-setup.sh 격리 + AC 트리거 좁힘).
