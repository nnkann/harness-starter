...
**Acceptance Criteria**:
- [x] Goal: tag 정규식 위반 auto-fix 도구를 도입해 다운스트림 누적 부채 마찰 해소 (S7 cluster 간선 정합 유지, S9 매칭 100% 보조)
  검증:
    tests: pytest .claude/scripts/tests/test_tag_normalize.py -v (15케이스 통과)
    실측: `python .claude/scripts/docs_ops.py tag-normalize <path>` 실행 가능 + dry-run 출력 + `--apply` 시 frontmatter 갱신. pre-check 차단 메시지에 auto-fix 명령 안내 존재 (grep "auto-fix" .claude/scripts/pre_commit_check.py)
- [x] `normalize_tag()` 결정적 변환 규칙 5종 (camelCase·언더바·대문자·비허용·중복) 구현
- [x] 한글 포함 tag 자동 변환 거부 — None 반환 + skip
- [x] pre_commit_check.py tag 위반 차단 메시지에 auto-fix 명령 한 줄 추가
- [x] 결정 박제 (본 섹션 변경 이력) — SSOT 위치 + 변환 규칙 + 한글 정책 명시

## 변경 이력

### v0.47.6 (2026-05-15) — tag-normalize 도구 도입 (FR-X2)

- **배경**: 다운스트림 v0.42.7→v0.47.4 적용 보고 — 다운스트림 7개 문서 tag 정규식 위반 누적. v0.47.1 정규식 도입 시점에 작성된 incidents·decisions가 대문자·언더바·한글 사용. 매번 사용자가 일일이 lowercasing → 마찰
- **반영 위치**: `.claude/scripts/docs_ops.py` `tag-normalize` 서브커맨드 신설, `.claude/scripts/pre_commit_check.py` tag 위반 차단 메시지에 auto-fix 명령 안내 추가
- **변환 규칙** (`normalize_tag()` 결정적):
  - camelCase·PascalCase → 경계 하이픈 삽입 + 소문자 (`ArtistMatcher` → `artist-matcher`, `csoonId` → `csoon-id`)
  - 언더바 → 하이픈 (`task_groups` → `task-groups`)
  - 대문자 → 소문자 (`REVIEW` → `review`)
  - 비허용 문자 → 하이픈 + 연속 압축 + 시작·끝 strip
  - **한글 포함 → 자동 변환 거부** (`None` 반환). 사용자 영문 매핑 필요
- **CLI 동작**:
  - `python .claude/scripts/docs_ops.py tag-normalize [<path>]` — dry-run 기본 (변경 없음)
  - `--apply` — 실제 frontmatter 갱신 + `updated:` 박제
  - `--yes` — 비대화형 (한글 tag는 자동 skip)
  - 한글 포함 파일은 `--apply`해도 본문 보존 + skip 메시지
- **pre-check 안내**: tag 위반 차단 메시지 다음 줄에 `auto-fix: ... tag-normalize <wip> --apply` 한 줄 추가. 사용자 즉시 실행 가능 (수동 trigger, 자동 실행 안 함)
- **한글 자동 변환 금지 이유**: `가격재수집` → `price-recollection`? `price-rescrape`? — 도메인 문맥 없이 모호. transliteration은 무의미. 사용자 결정 필수
- **검증**: `pytest .claude/scripts/tests/test_tag_normalize.py` 15케이스 통과 — 정규화 결정성 + dry-run/apply/한글 skip/중복 dedup/정합 보존 5축
- **P11 동형 잠복 후보** (별 wave 정련): pre-check 차단 항목이 차단만 하고 auto-fix 없는 패턴 — TODO/FIXME 차단, S# 미인용 차단, AC 체크박스 부재 차단. completed 봉인 차단은 이미 `docs_ops.py reopen` 안내 있음 (긍정 사례 — 본 wave 안내 패턴이 그 사례 follow)
