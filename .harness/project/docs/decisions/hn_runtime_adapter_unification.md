...
problem: [P3, P5, P7]
s: [S3, S5, S7]
tags: [runtime, adapter, hermes, codex, downstream]
status: completed
created: 2026-05-26
updated: 2026-06-01
relates-to:
...
- `agy-review.sh`를 downstream 공통 Agy advisory runner로 둔다. Agy 인증·모델·실행 파일은
  repo에 고정하지 않고 local execution binding(`AGY_BIN` 또는 PATH)으로 둔다.
- `agy-review.sh`는 Agy state dir 쓰기 가능성을 먼저 확인한다. Codex tool sandbox처럼
  `~/.gemini/antigravity-cli`에 쓸 수 없는 환경이면 실행을 중단하고 로컬 터미널 handoff를 안내한다.
- 로컬 Agy 실행 결과는 `.claude/memory/session-agy-review.md`에 저장한다. Codex는 이
  gitignore된 session handoff 파일을 읽어 Agy 의견을 같은 작업 흐름에 반영한다.
...
- `python3 -m pytest .claude/scripts/tests/test_agy_review.py -q`
...
- [x] S3/S7: downstream마다 같은 방식으로 Agy advisory review를 호출할 수 있도록
  `.claude/scripts/agy-review.sh` 공통 runner와 readiness 관측을 추가한다.
- [x] S3/S7: Codex tool sandbox에서 Agy state dir 쓰기가 불가능하면 runner가 Agy를
  실행하지 않고 같은 project root의 로컬 터미널 handoff를 안내한다.
- [x] S3/S7: 로컬 Agy 응답을 `.claude/memory/session-agy-review.md`에 저장해
  Codex가 같은 작업에서 회수할 수 있게 한다.
- [x] S3/S7: Agy가 의사결정 advisor로 적극 개입하도록 runner 기본값을 full
  permission으로 두고, readiness/README/MIGRATIONS/test가 같은 계약을 검증한다.
- [x] `.claude/skills`와 `.agents/skills` drift audit 경로를 `hn_runtime_ssot_generation`,
  `hn_codex_port`, `test_codex_agents.py`, `downstream-readiness.sh`로 분리한다.
- [x] StageLink의 Hermes manifest/worker 설정과 guardian 경로가 Hermes-managed
  downstream 운영 문서에 반영된다.

## 결정 사항

- 2026-05-30: Agy는 repo-local adapter 파일을 복사해 고정하지 않고, 모든 downstream에서
  `.claude/scripts/agy-review.sh`를 통해 호출한다. runner는 현재 프로젝트 root를
  `--add-dir`로 넘기고, 실제 Agy binary는 `AGY_BIN` 또는 PATH의 local execution binding으로 찾는다.
- 2026-05-30: Codex tool sandbox 안에서 Agy를 실행하면 `~/.gemini/antigravity-cli`
  상태 저장이 막혀 장기 세션·continue·artifact 기반 운용이 깨질 수 있다. runner가
  write probe 실패 시 exit 73으로 중단하고 로컬 터미널 직접 실행 명령을 출력한다.
- 2026-05-30: Agy 응답 회수는 대화 붙여넣기 대신 `.claude/memory/session-agy-review.md`
  handoff 파일을 기본으로 한다. 파일은 `.gitignore`의 `.claude/memory/session-*`
  패턴에 걸려 커밋되지 않는다.
- 2026-05-31: Agy는 단순 읽기 전용 보조가 아니라 의사결정에 적극 개입하는
  advisor로 둔다. `agy-review.sh` 기본값은 `AGY_PERMISSION_MODE=full`이고
  `--dangerously-skip-permissions`를 붙여 실행한다. 제한 환경만 명시적으로
  `AGY_PERMISSION_MODE=prompt`로 낮춘다.
- 2026-05-31: Codex의 전역 승인 정책은 하네스 파일만으로 보장할 수 없다.
  하네스가 보장하는 범위는 downstream에 복제되는 Agy runner 계약과 readiness
  관측 신호다.

## 정리 결과

본 결정은 하네스 방향과 맞다. 현재 하네스의 중심은 특정 runtime이 아니라
Harness core contract + runtime adapter + local execution binding 분리다.

- `hn_runtime_ssot_generation`: runtime surface를 생성/검증 대상으로 보는 이전
  결정이다. 본 문서가 다중 runtime stack과 Hermes/Codex/Agy 기본 조합을 구체화한다.
- `hn_codex_port`와 `test_codex_agents.py`: `.agents`/`.codex` bridge의 stale
  reference와 역할 drift를 회귀 테스트로 관리한다.
- `downstream-readiness.sh`: `runtime_stack`과 `runtime_adapters`를 downstream
  관측 신호로 출력한다.
- `hn_hermes_managed_downstream_memory`: StageLink guardian/manifest 운영 경계를
  별도 downstream 운영 결정으로 보존한다.

남는 일은 새 WIP가 아니라 후속 구현 트리거다. adapter drift가 다시 관측되면
`hn_runtime_ssot_generation` 계열을 재개하고, StageLink manifest 차이는
downstream 쪽 WIP로 처리한다.
