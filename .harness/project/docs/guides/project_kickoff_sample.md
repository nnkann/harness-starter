...
### User Needs
> 선택적 섹션. "왜 만드는가"를 사용자 관점으로 보존. 작업이 쌓일수록
> 이 섹션이 "원래 목표"로 돌아오는 기준점이 된다.

#### Personas
- **팀 리더**: 작업 할당·진행률 파악이 목적. 지금은 슬랙 DM + 스프레드시트로 해결 중.
- **팀원**: 자기 작업 확인·상태 갱신이 목적. 다른 팀원 상태는 관심 없음.

#### Success Metrics
- 팀 리더가 전체 작업 상태를 5초 이내에 파악할 수 있다.
- 팀원이 별도 도구 없이 상태를 갱신할 수 있다.
- 주 1회 이상 실제로 사용된다 (도입 후 1개월 기준).
