...
# Handoff 1: hermes-kann and ha_thoth
...
has('docs/harness/agents/ha_thoth.md',['compile root goal into CPS expression','define task_AC','define owner_approval_boundary','define prohibited_actions','define evidence_acquisition.C/P/S','identify required_docs','identify doc_ops_needed','propose actor_binding','implementation','commit/push','raw stdout/git/log/sqlite/test dump prefetch','direct Honcho policy write without source_ref'],'ha_thoth full contract')
# Handoff 2
has('docs/harness/agents/ha_maat.md',['cps_audit_and_project_digest_gate','project-specific skill digest','allowed evidence request','count','path','top_error','line_ref','artifact_ref','schema_field_presence','full raw stdout','full git diff/log','full sqlite dump','full test output','full skill/document dump'],'ha_maat audit+skill digest gate')
for agent in ['ha_ptah','ha_anubis','ha_sekhmet']:
...
has('docs/harness/agents/ha_seshat.md',['create/update required md files','enforce frontmatter schema','maintain docs in git as source of truth','generate doc_ops_manifest','generate honcho_ingest_manifest','track source_path, line_ref, commit_ref, doc_type','detect stale or missing required docs','prepare digest artifacts for Honcho ingestion'],'ha_seshat doc_ops')
# Handoff 3 required docs
for path in ['docs/harness/contracts/cp_frontmatter_schema.md','docs/harness/contracts/cp_cps_evidence_acquisition.md','docs/harness/contracts/cp_owner_approval_boundary.md','docs/harness/contracts/cp_agent_role_contracts.md','docs/harness/contracts/cp_kanban_promotion_contract.md','docs/harness/contracts/cp_honcho_doc_wiki_boundary.md']:
    exists(path,'required contract '+path)
for path in ['docs/harness/agents/ha_thoth.md','docs/harness/agents/ha_maat.md','docs/harness/agents/ha_seshat.md','docs/harness/agents/ha_ptah.md','docs/harness/agents/ha_anubis.md','docs/harness/agents/ha_sekhmet.md','docs/harness/agents/ha_honcho_archivist.md','docs/harness/agents/ha_honcho_librarian.md','docs/harness/agents/ha_honcho_context.md']:
...
for agent,terms in {
'ha_honcho_archivist':['ingest required md digest into Honcho','preserve source_path/source_commit/line_ref/artifact_ref','classify doc_type','store frontmatter_summary','avoid raw stdout/log archival'],
'ha_honcho_librarian':['verify required md files are indexed','detect stale docs','compare repo source vs Honcho digest','flag missing CPS/frontmatter/evidence sections','report drift'],
'ha_honcho_context':['retrieve project-relevant docs','provide source_ref candidates','suggest prior CPS patterns','provide context without overriding Harness policy','Honcho must not override Harness policy'],
}.items():
...
has('.harness/hermes/doc-generation.yaml',['ha_seshat','ha_maat','ha_honcho_archivist','ha_honcho_librarian','ha_honcho_context','native_auto_decompose_for_harness_boards: false','harness_promotion_compiler_required: true'],'workflow lifecycle bindings')
...
has('docs/harness/contracts/cp_honcho_agent_management.md',['managed_agents','ha_honcho_archivist','ha_honcho_librarian','ha_honcho_context','authority_order','Honcho-only policy creation'],'Honcho agent management contract exists and is managed')
