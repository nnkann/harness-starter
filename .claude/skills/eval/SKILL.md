---
name: eval
description: 놓치고 있는 것을 찾는다. 기본은 코드 간극 분석. --harness는 하네스 문서 품질 평가. --surface는 암묵지 발견. --deep을 붙이면 3관점 2차 검증. 주기적으로 돌리는 건강 검진.
---
...
| `/eval --surface` | 암묵지 발견 |
| `/eval --deep` | 기본 + 3관점 2차 검증 |
| `/eval --harness --deep` | 하네스 품질 + 3관점 2차 검증 |
...
2. 1차 결과를 보고한다.
3. 이어서 3관점으로 2차 검증을 실행한다.
4. 각 관점에서 발견한 것만 보고한다. 없으면 해당 관점은 생략.
