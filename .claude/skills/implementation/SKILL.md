...
### Step 0.7. 규모·위험도 분기 (라우팅 결정)

사용자 발화 + 예상 변경 범위로 판정. 이후 Step 1~2.5의 호출 강도 결정.

| 규모 | 기준 | WIP | specialist 호출 |
|------|------|-----|-----------------|
| micro | 단일 파일, <20줄, 리스크 낮음 | 생략 | 없음 (commit이 검증) |
| small | 단일 파일 20~100줄 또는 다중 파일 <3개 | 생성 | doc-finder 1회 |
| medium | 3~10 파일 또는 1 파일 <300줄 구조 변경 | 생성 | doc-finder + codebase-analyst, 위험도 hit 시 risk-analyst |
| large | 10+ 파일 또는 핵심 구조 변경 | 생성 | advisor 필수 (PM이 specialist pool 오케스트레이션) |

**위험도 hit 신호**: 공개 API 변경, 인증/보안 경로, DB 마이그레이션, 되돌리기
어려운 결정, 핵심 설정(CLAUDE.md/rules/settings.json) 구조 변경.

large 시 advisor 응답은 Step 1에서 만드는 WIP `## 메모`에 전문 기록.
...
작업 중 막히면 (`no-speculation.md` 준수):

1. **관찰·재현·선행 사례 확인** — 첫 행동은 이 셋 중 하나
2. **3회 시도 규칙** — 같은 접근법이 3회 실패하면 접근법 자체를 의심.
   specialist 재호출 또는 advisor 스킬로 전환
3. **사용자 보고** — 추측으로 넘어가지 말고 현재 상태·시도 내역·막힌 지점을
   보고. "이걸로 될까요?"로 검증 책임 떠넘기기 금지
4. **중단** — 복구 불가 판단 시 WIP status `in-progress` → `abandoned`,
