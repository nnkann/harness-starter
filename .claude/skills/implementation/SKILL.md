...
> "하네스 초기화가 아직 완료되지 않았습니다. CPS와 기술 스택이 없으면 작업 방향을 검증할 수 없습니다.
> (1) `/harness-init`을 먼저 실행 — 권장
> (2) 이번 작업만 init 없이 진행
> (3) 이 프로젝트에서는 CPS를 사용하지 않음"

사용자가 (2)를 선택하면 이 단계를 건너뛴다.
사용자가 (3)을 선택하면 이 단계를 건너뛰되, 선택을 기억한다 (feedback 메모리에 저장).

**init 완료 상태:**
docs/setup/에 CPS 문서(`project_kickoff_*.md`)가 있으면 먼저 읽는다.
