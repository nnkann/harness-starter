...
### 약어 표 검증 (naming.md "도메인 약어")

매 `--validate` 실행 시 추가:

| 검사 | 규칙 |
|------|------|
| 약어 중복 | 두 도메인이 같은 abbr을 쓰면 에러 |
| 도메인-약어 1:1 | "도메인 목록 > 확정"의 모든 도메인이 약어 표에 있어야 함. 누락 시 경고 |
| 약어 형식 | 2~3자 소문자 영문만. 숫자·기호 포함 시 경고 |
| 파일명-abbr 매칭 | docs/ 문서 파일명 prefix가 등록된 abbr 중 아무 것과도 매치 안 되면 경고 (전역 마스터 예외) |
| 파일명 날짜 suffix | `_\d{6}\.md` 패턴 발견 시 경고 (전 폴더. naming.md 규칙 위반) |

### 파일명 규칙 위반 보고 형식

```
⚠️ 파일명 규칙 위반:
  - decisions/hn_memory_260420.md: incidents 외 날짜 suffix 금지
    → 제안: git mv decisions/hn_memory_260420.md decisions/hn_memory.md
  - guides/old_style.md: 도메인 abbr 없음 + 프론트매터 domain도 없음
    → 제안: abbr 추가하거나 전역 마스터 문서라면 프론트매터 domain 설정
```
...
1. **이동 전 차단 검사**: status를 `completed`로 전환할 때 본문에 차단
   ...
   실행 절차 (순서대로). **차단 키워드 목록 SSOT는 `.claude/rules/docs.md`
   "## completed 전환 차단" 섹션**. 아래 grep 패턴은 그 SSOT를 실행하는
   구현이며, 키워드 추가·변경은 rules/docs.md에서 먼저 한다.
   ...
   # 2. 차단 키워드 검사 (rules/docs.md 키워드 목록 구현, 인용·완료 표시 제외)
   #    - "## 후속" 같은 헤더는 진짜 미결
...
3. `git mv`로 이동 (접두사 제거). 최종 파일명:
   - 일반: `{abbr}_{slug}.md` (모든 대상 폴더)
   - 전역 마스터: `{slug}.md`
   **날짜 suffix 전면 금지** (naming.md 규칙). 이동 대상 파일명에
   `_{YYMMDD}` 있으면 경고·거부.
...
## Step 3. clusters/ 갱신 — 직교 파싱 기반 자동 매핑

docs/clusters/{domain}.md를 갱신한다. **문서 → cluster 매핑은 파일명의
abbr을 직교 파싱**해 자동 결정 (SSOT: `.claude/rules/naming.md` "Cluster
자동 매핑" 섹션).

### 파싱 절차

```
1. naming.md "도메인 약어" 표에서 abbr 목록 수집 → [hn, mt, ...]
2. 대상 파일명에서 패턴 (^|[_-])(<abbr>)_ 검색 (가장 앞쪽 매치 사용)
3. 매치되면 대응하는 clusters/{domain}.md에 등록
4. 매치 없으면 프론트매터 domain: 으로 폴백 (전역 마스터 문서)
```

### 구현 예시 (bash)

```bash
# naming.md에서 약어 목록 추출 (표 파싱)
ABBRS=$(awk '/^\| [a-z_]+/{gsub(/\|/," "); print $2}' .claude/rules/naming.md \
        | tr '\n' '|' | sed 's/|$//')
# 예: ABBRS="hn|mt|cs|..."

# 대상 파일명에서 abbr 추출 (가장 앞쪽 매치)
FILENAME="m3-hn_t04_redesign.md"
BASENAME="${FILENAME%.md}"
DETECTED=$(echo "$BASENAME" | grep -oE "(^|[_-])(${ABBRS})_" | head -1 \
           | sed -E "s/^[_-]?(${ABBRS})_$/\1/")
# DETECTED="hn"
```
...
| 파일명 | 감지 abbr | cluster |
|--------|-----------|---------|
| `decisions/hn_memory.md` | hn | clusters/harness.md |
| `incidents/hn_leak_260421.md` | hn | clusters/harness.md |
| `WIP/m3-hn_t04_redesign.md` | hn | clusters/harness.md (불투명 prefix 통과) |
| `WIP/decisions--hn_memory.md` | hn | clusters/harness.md (라우팅 통과) |
| `m3-hn_t04_mt_review.md` | hn | clusters/harness.md (첫 매치 정책) |
| `guides/project_kickoff.md` | (없음) | 프론트매터 `domain:` 폴백 |

### cluster 갱신 내용

- 해당 도메인의 모든 문서 목록 (WIP 제외)
- 관계 맵 (relates-to 기반)
- 새 domain이 추가되었으면 cluster 파일 생성 (naming.md 도메인 목록 +
  약어 표에 모두 등록되어야 함)
