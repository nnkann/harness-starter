...
description: 하네스 스타터 업그레이드 후 충돌 파일을 대화형으로 병합. setup.sh --upgrade 실행 후 .claude/.upgrade/ 에 스테이징된 파일들을 분석하고 사용자 승인 하에 병합. "harness-upgrade", "하네스 업그레이드", "업그레이드 병합" 요청 시 사용.
---

# harness-upgrade 스킬

`setup.sh --upgrade` 실행 후, 스테이징된 변경 파일들을 대화형으로 병합한다.

## 전제

- `setup.sh --upgrade`가 이미 실행되어 `.claude/.upgrade/` 디렉토리가 존재해야 한다.
...
- [ ] `.claude/.upgrade/` 디렉토리 존재 확인. 없으면:
  ...
  먼저 harness-starter에서 `bash setup.sh --upgrade <이 프로젝트 경로>`를 실행하세요.
  ```
...
### Step 2. settings.json 특별 처리

settings.json은 가장 복잡한 병합 대상이다.

전략:
1. 현재 settings.json을 JSON으로 파싱
2. 새 settings.json을 JSON으로 파싱
3. 카테고리별 비교:
   - `PreToolUse` 훅: matcher 기준으로 비교. 새 matcher가 있으면 추가.
   - `PostToolUse` 훅: 동일.
   - `Stop` 훅: 현재에 없으면 추가.
   - `PostCompact` 훅: 현재에 없으면 추가.
4. 기존 훅의 command/prompt가 변경된 경우:
   - 양쪽 버전을 보여주고 사용자가 선택
5. 사용자가 추가한 커스텀 훅(스타터에 없는 matcher):
   - 무조건 보존
