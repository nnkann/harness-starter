...
pre_commit_check가 출력하는 `promotion` 값으로 분기 (release path 진입
여부의 SSOT). is_starter 단독은 트리거가 아니다 — release 파일 staging
여부와 결합해 `promotion=release`가 발화될 때만 실행.
...
- `promotion=repair` → 현재는 none과 동일 취급 (hook SSOT 통합 시 활성화).
...
- **side effect ledger**: pre-check stdout의 `side_effects.required`,
