...
보관하되, **review prompt에는 allowlist grep으로 10 keys만 추출해 박는다**:
`already_verified`, `risk_factors`, `diff_stats`, `signals`, `domains`,
`domain_grades`, `multi_domain`, `repeat_count`, `recommended_stage`,
`s1_level`. 나머지 4개(`pre_check_passed`, `split_plan`,
...
# review prompt용 — allowlist로 10 keys만 추출 (split_group_N_* 동적 키 차단)
REVIEW_PRECHECK=$(echo "$PRE_CHECK_OUTPUT" | grep -E \
  "^(already_verified|risk_factors|diff_stats|signals|domains|domain_grades|multi_domain|repeat_count|recommended_stage|s1_level):")
```
