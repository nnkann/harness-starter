...
| Pass (pre-check→나) | `wip_problem` · `wip_solution_ref` · `ac_tests` · `ac_actual` · `recommended_stage`(default 또는 deep) · `s1_level` (스키마 SSOT: `pre_commit_check.py`) |
| Pass (나→review) | 아래 "review 호출" 섹션이 이 Pass 블록을 조립한다 (self-containment) |
| Preserve | pre-check stage·wip_problem·wip_solution_ref 원본 (재계산 금지) · 사용자 플래그 |
| Signal risk | ⛔ pre-check 3회 연속 차단·시크릿 line-confirmed · 🔍 검증 흔적 |
| Record | commit log `🔍 review: <on\|off\|forced> \| problem: P# \| s: S#` 한 줄 |
...
| 사용법 | 설명 |
|--------|------|
| `/commit` | review 안 함 (default). pre-check 통과 + 시크릿 게이트만. |
| `/commit --review` | review agent 호출. verdict 강제 없음, diff별 한 줄 의견. |
| `/commit --no-review` | review 명시 스킵 (커밋 메시지에 `[skip-review]` 태그). |

> **stage 5단계 폐기 (2026-05-14 §S-2 73% 삭감)**: skip/micro/standard/deep
> 자동 분류 + `--quick`/`--deep`/`recommended_stage` 폐기. AC `검증.review`
> 5단계 자가 선언이 분류 의미를 못 가졌음. review 호출은 사용자 플래그
> 2단계로 단순화. 보안 게이트(시크릿 line-confirmed)만 `--no-review`
> 무시하고 review 강제.

## 리뷰는 스킬이 Agent tool로 직접 호출한다

코드 리뷰는 이 스킬이 `Agent` tool로 review 서브에이전트(`.claude/agents/review.md`)를
직접 호출한다. hook은 쓰지 않는다. 호출 여부는 **사용자 플래그**로 결정.
...
Agent tool 호출
  ...
  prompt: 아래 블록 5개를 그대로 포함
    ...
    4. ## pre-check 결과 (wip_problem·wip_solution_ref·s1_level·already_verified)
    5. ## 지시 (verdict 강제 없음, diff별 한 줄 의견)
```
...
## 지시
WIP AC를 검증 기준으로 삼아라.
**AC + 전제 컨텍스트만으로 판단 가능하면 즉시 의견 출력 — 파일 Read 금지.**
의심점이 명확할 때만 Read/Grep을 사용하되 **Read+Grep 합계 3회 이내**.
스코프 이탈 의심 시에만 `git diff --cached` 1회.

## 출력 형식
diff별 한 줄 의견. verdict 단어 강제 없음. 명확한 차단 사유 있으면
첫 줄에 명시. 그 외는 자유 서술.
...
wip_problem: P#                                   # frontmatter problem
wip_solution_ref: S2; S6                          # frontmatter s 번호 list (";" 구분)
ac_tests: <pytest 명령 또는 "없음">                # AC 검증.tests 값
ac_actual: <명령 또는 "운용 검증">                  # AC 검증.실측 값
recommended_stage: default|deep                   # default=사용자 플래그 결정, deep=시크릿 강제
...
- `ac_review` (2026-05-14 §S-2 — `검증.review` 자가 선언 폐기)
- stage 5단계 (skip/micro/standard/deep — 2단계 default/deep로 단순화)
...
- commit 스킬은 **첫 그룹만으로** Step 6·7·7.5·8(커밋)·9(푸시 제외) 수행
  ...
  - sub-커밋 review는 사용자 플래그 그대로 (부모와 동일). stage 5단계 분기 폐기
...
**호출 결정 (단순 2단계 — 2026-05-14 §S-2)**:

| 조건 | review 호출 |
|------|------------|
| `--no-review` 플래그 | skip (단 보안 게이트는 무시) |
| `--review` 플래그 | 호출 |
| 플래그 없음 (default) | skip |
| `recommended_stage: deep` (시크릿 line-confirmed) | **강제 호출** (--no-review 무시) |

stage 5단계(skip/micro/standard/deep) 분류 폐기. verdict 단어 강제 추출 폐기.
review는 호출되면 diff별 한 줄 의견만 반환. 차단은 사용자가 응답 보고 판단.
...
verdict 강제 추출 폐기. review 응답 본문을 사용자에게 그대로 전달.
사용자가 응답 보고 다음 행동 결정:
- 응답에 명확한 차단 사유 있음 → 사용자가 commit 중단 + 수정 후 재시도
- 응답이 경고·제안 수준 → commit 메시지 본문에 요약 반영하고 진행
- 응답이 무해 → 그대로 진행
...
review 호출 직전 한 줄 알림:
> 🔍 review: --review 플래그 (또는 시크릿 게이트 강제)

다음 커밋에서 `--no-review` 플래그로 옵트아웃 가능 (보안 게이트 hit 시는 무시).
...
- 시크릿 line-confirmed (보안 게이트 deep)
