...
자동 매칭 + ✅ 표시는 Step 7 review pass 직후 (staged 확정 최종 상태)
에서 수행한다.
...
### 4. 하네스 버전 체크 + 연동 갱신 (release path)

> **release path 전용 단계.** 일반 fast path는 이 Step을 건너뛴다.

§H-1에서 pre_commit_check가 출력하는 `promotion` 값으로 분기 (release path
진입 여부의 SSOT). is_starter는 더 이상 단독 트리거가 아니다 — release
파일 staging 여부와 결합해 `promotion=release`가 발화될 때만 실행.

**Step 5의 pre-check이 끝난 뒤** 본 Step을 다시 평가하므로, 본 Step 본문은
Step 5 직후 위치에서 실행되거나 Step 5 stdout을 미리 가져와 분기해도 된다.
구현 흐름:

```bash
PROMOTION=$(echo "$PRE_CHECK_OUTPUT" | sed -n 's/^promotion: //p')
case "$PROMOTION" in
  release) ;;          # 계속 진행
  none|repair|"")
    # release 아님 — version bump·MIGRATIONS·README 전부 skip
    # Step 5 또는 다음 단계로 진행
    ;;
esac
```

- `promotion=none` (일반 fast path) → **Step 5/이후로 즉시 진행. 아래 내용 실행하지 않는다.**
- `promotion=repair` → 본 wave 범위 외 (§H-5에서 활성화). 현재는 none과 동일 취급.
- `promotion=release` → 계속:

> **promotion=release 발화 조건** (`.claude/scripts/pre_commit_check.py` SSOT):
> `is_starter=true` AND staged 파일에 `.claude/HARNESS.json` / `docs/harness/MIGRATIONS.md` / `README.md` 중 하나 이상. 셋 모두 없으면 fast path. 다운스트림(`is_starter=false`)은 항상 promotion=none → 본 Step skip.

> **Step 3 이후 실행 필수** — 스크립트가 staged 파일을 읽어 범프 타입을 결정한다.
> 스테이징 전에 실행하면 항상 "staged 없음"으로 오판.
...
### 5.5. 커밋 분리 판정 (1 커밋 = 1 논리 단위)

pre-check stdout의 **`commit_route` 가 1차 신호**다. `split_action_recommended`는
보조 신호로만 사용한다 — fast path에서는 분리 권고가 있더라도 사용자가
명시 요청하지 않으면 single로 진행하는 게 기본.

```bash
COMMIT_ROUTE=$(echo "$PRE_CHECK_OUTPUT" | sed -n 's/^commit_route: //p')
SPLIT_REC=$(echo "$PRE_CHECK_OUTPUT" | sed -n 's/^split_action_recommended: //p')
```

분기:

- **`commit_route=sub`**: 현재 sub-커밋 진행 중 (`HARNESS_SPLIT_SUB=1`). Step 6으로 진행
- **`commit_route=single` + `split_action_recommended` ∈ {single, sub}**: 분리 불필요. Step 6으로 진행
- **`commit_route=single` + `split_action_recommended=split`**: 분리 권고. 아래 흐름은 **사용자 명시 요청 시에만** 실행. `HARNESS_SPLIT_OPT_IN=1` 또는 명시 분리 요청 없으면 single 진행 + 커밋 메시지에 `AC-MIXED` 태그 (staging.md "## split 옵트인 정책" SSOT).

#### split 흐름

split-commit.sh는 **비파괴 planner가 기본**. 사용자가 명시 분리 요청한
경우에만 `--apply`로 진입.
...
커밋 정책은 모두 `.claude/rules/staging.md` 참조. 이 스킬은 pre-check stdout
의 **`review_route` 를 1차 신호**로 받아 review를 호출한다. 옛 `recommended_stage`
는 호환성 폴백 — `review_route`가 비었을 때만 사용.

```bash
REVIEW_ROUTE=$(echo "$PRE_CHECK_OUTPUT" | sed -n 's/^review_route: //p')
# 폴백 (호환성)
[ -z "$REVIEW_ROUTE" ] && REVIEW_ROUTE=$(echo "$PRE_CHECK_OUTPUT" | sed -n 's/^recommended_stage: //p')
```

값 매핑: `skip|micro|standard|deep` 그대로. 플래그 오버라이드(`--no-review`/
`--quick`/`--deep`)는 staging.md 우선순위 적용 후에도 `review_route` 변수에
최종값을 박는다 (이후 호출 분기 단일화).
...
`git commit` 직접 호출 금지. `commit_finalize.sh` wrapper 경유 — 내부에서
wip-sync → git commit 단일 흐름 처리.
...
> **강제 경유 규약**: `bash-guard.sh` 검증 4가 `git commit` 직접 호출을
> 차단한다. **반드시 `HARNESS_DEV=1` prefix를 앞에 붙여 실행**해야 통과.
> prefix 누락 시 bash-guard가 exit 2. 이스케이프 경로는 `HARNESS_DEV=1` 단일.
...
- **side effect ledger** (§H-2~): pre-check stdout의 `side_effects.required`,
  `side_effects.release`, `side_effects.repair` 3줄을 그대로 출력. 각각의
  값이 `none`이 아니면 사용자가 본 커밋이 만든 자동 변경의 분류를 즉시 본다.
  예:
  ```
  side_effects.required: docs_ops.wip-sync
  side_effects.release: .claude/HARNESS.json,README.md,docs/harness/MIGRATIONS.md
  side_effects.repair: none
  ```
  세 값 모두 `none`이면 요약 라인 자체를 생략 가능 (노이즈 감소).
