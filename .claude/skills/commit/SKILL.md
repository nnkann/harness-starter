...
Step 4의 변경 내역 분석 결과를 바탕으로, 아래 조건 중 **하나라도** 해당하면
strict 모드의 Review 검증(5S)을 자동 실행한다.
...
**해당 없으면** → 리뷰 없이 5L로 진행.
**해당하면** → 사용자에게 "⚡ 위험도 감지: [해당 조건]. 리뷰 에이전트를 실행합니다."를 알린 후 5S 실행.
리뷰 결과에 **차단** 이슈가 있으면 커밋 중단, 아니면 이어서 5L로 진행.
