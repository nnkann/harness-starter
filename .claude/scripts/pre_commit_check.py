...
def main() -> int:
    ...
    # HARNESS.json은 main 함수 내 1회만 read. 라인 768·997 중복 제거.
    _harness_cache: dict | None = None
    try:
        import json as _json
        _harness_cache = _json.loads(Path(".claude/HARNESS.json").read_text(encoding="utf-8"))
    except Exception:
        _harness_cache = None

    try:
        _harness = _harness_cache if _harness_cache is not None else {}
    ...
    if session_file.exists():
        # Phase 4 (hn_harness_recovery_v0_41_baseline, 2026-05-13):
        # git diff --cached --name-only 중복 호출 제거.
        # staged_files는 라인 299에서 name-status 파싱 결과로 이미 존재.
    ...
    try:
        # Phase 4: 위에서 캐싱한 _harness_cache 재사용.
        hj = _harness_cache if _harness_cache is not None else {}
