...
c: 작업 시작 맥락 1줄           # 권장. C-P-S-AC 연결 원자료
...
- **c**: C-P-S-AC 연결 원자료. 사용자 발화·실측·관찰을 1줄로 보존한다.
...
**원칙**:
- 매칭 강제 없음. 자라는 시스템
- `c`는 날것의 관찰이다. P#가 나중에 바뀌어도 재분류할 수 있게 보존한다.
...
| 키 | 값 | 의미 |
|----|----|----|
| `review` | `skip` | 리뷰 불필요. 근거가 `실측`에 있어야 함 |
| `review` | `self` | 작성자 자체 검토 |
| `review` | `review` | review 에이전트 1회 |
| `review` | `review-deep` | cascade·보안·공개 API 등 고위험 변경 |
...
- 기본: `/commit` — review 호출 없음. pre-check + 시크릿 게이트만 실행
- 옵트인: `/commit --review` — review agent 1회 호출, verdict 강제 없음
- 명시 스킵: `/commit --no-review` — review 스킵 의도를 커밋 흐름에 남김
