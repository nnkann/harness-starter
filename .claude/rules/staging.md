...
3. **검증 기준은 WIP AC에서 온다.** review는 diff 전에 WIP task의 `Goal:`
   항목을 먼저 읽는다. AC에 명시된 범위가 deep 검증 스코프를 결정한다.

## 신호 (13개)

상세 패턴은 `pre_commit_check.py`가 SSOT.
...
- `h-setup.sh` — 설치·업그레이드 진입점. 버그 시 다운스트림 전체 설치 파괴
...
- **AC `영향 범위:` 항목 1개 이상** → deep 트리거 (kind 기반 판단 이후 2차 격상)
...
- **AC 전부 [x] + 영향 범위 항목 없음 + S6/S7 단독** → micro 완화 가능
...
- `pre_commit_check.py`: 신호 감지 SSOT + stdout 스키마
