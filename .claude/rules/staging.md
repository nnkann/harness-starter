...
| 신호 | 정의 |
|------|------|
| S1 | 보안·시크릿 — 시크릿 패턴 hit. **2단계 강도 분리**: `s1_level=line-confirmed`(실제 시크릿 패턴)는 deep, `s1_level=file-only`(파일명만 hit)는 standard. 테스트·docs·example·`-helper.`/`-utils.` 파일은 면제 |
...
```
0a. 메타·문서 단독은 도메인 등급 무시 (4~7·12~13으로 처리)
1. S9(critical) + (S7·S2·S8·S14 동반) → Stage 3
   ※ critical 도메인이라도 메타·문서 단독(S5/S6만)이면 룰 1 미적용,
     아래 4~7·12~13으로 떨어진다 (incident: doc-only deep 과잉 호출)
2. S1(line-confirmed) hit → Stage 3
3. S2·S8 hit → Stage 3
3a. S14 hit → Stage 3 (마이그레이션은 무조건 deep)
3b. S1(file-only) hit → Stage 2 (파일명만 hit, 라인 패턴 없음)
...
13. S6만 + strict → Stage 1 (critical 도메인 포함 — 문서 변경엔 micro 충분)
...
s1_level: ""|file-only|line-confirmed   # S1 강도 (파일명만 vs 라인 패턴 확정)
```
