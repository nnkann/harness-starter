...
### downstream cron 학습 신호

Hermes cron·downstream guardian·manual executor report는 repo-local memory보다
상위 운영 신호다. report가 "진행 안 됨", "delta 미반영", "upgrade 가능",
"candidate-upstream-change"를 말하면 다음 순서로 처리한다.

1. 현재 WIP의 `domain/problem/s/tags`와 맞으면 WIP `## 메모`에 report 경로와
   재확인 결과를 흡수한다.
2. 관련 WIP가 없고 독립 판단 단위이면 `/implementation` WIP로 승격한다.
3. 반복 절차 자체는 Hermes skill/cron manifest owner로 남기고, repo에는
   프로젝트 SSOT·정책·테스트 가능한 계약만 둔다.
4. report 본문은 사실 증거가 아니라 `memory-signal`이다. repo 명령 결과,
   downstream 문서, git 상태 중 하나로 재확인하기 전 AC 증거로 쓰지 않는다.

cron 미진행 자체도 신호다. "사용자가 cron을 요청했는데 결과가 없다"는
운영 delta로 보고, Hermes-managed downstream 계약 또는 현재 WIP로 흡수한다.
자동으로 downstream 파일을 수정·커밋·푸시하지 않는다.
