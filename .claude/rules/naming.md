...
- 파일명 `hn_memory.md`만 봐도 **도메인(harness) + 주제(memory)** 확정.
  문서 열지 않고도 성격 파악 가능
- cluster 파일(`clusters/{domain}.md`)은 docs-manager가 파일명 abbr을
...
도메인마다 약어 1개가 등록**돼야 한다. 누락 시 docs-manager가 경고.
...
- `{대상폴더}--`: WIP 라우팅 태그 (decisions / guides / incidents / harness)
  `commit` 시 `docs-manager`가 제거하고 본 폴더로 이동
...
docs-manager가 파일명을 파싱해 cluster를 결정한다. **abbr이 파일명 어느
