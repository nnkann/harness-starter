...
함수·메서드·클래스·변수·상수·정규식·schema key·환경변수 이름이 같은 의미로
2곳 이상에 존재하면 코드 SSOT 후보로 본다.

- **owner 심볼 1곳**: 실제 판정·변환·기본값·정규식은 한 심볼이 소유한다.
- **호출부는 참조**: 같은 값/로직을 복붙하지 말고 import·호출·상수 참조로
  연결한다.
- **mirror는 명시**: 런타임 산출물·테스트 fixture·문서 예시는 mirror임을
  주석이나 문맥으로 드러낸다. mirror가 판정 SSOT처럼 행동하면 drift다.
- **변경 전 검색**: 기존 심볼을 추가·수정하기 전 이름/의미 키워드로 호출자와
  유사 심볼을 찾는다. `rg "def <name>|class <name>|<CONSTANT>|<schema_key>"`
  수준의 1차 확인 없이 새 심볼을 만들지 않는다.
- **불일치 발견**: 한쪽 값만 바꾸면 다른 위치가 낡는 구조를 발견하면 본 작업
  범위에 owner 지정·참조화·역할 분리 중 하나를 포함한다.

### 1. 3+ reference rule

0번은 같은 의미의 심볼이 갈라지는 drift를 막는 owner 지정 규칙이고, 1번은
동일 로직이 3곳째 반복될 때 core 추출을 의무화하는 리팩토링 기준이다.
...
## sub-task 분리 우회 금지 (v0.51.4 P12 흡수)

본 wave 영역 안에서 발견된 sub-task는 본 wave에서 처리한다. 별 WIP로
분리해 본 wave를 "외형만 완료" 처리 금지.

- **새 PR 원칙**: 새 PR 성격의 작업은 새 WIP를 만든다. 문서 구현 중 파생된
  수정은 같은 C·Goal·AC로 닫히면 같은 wave에서 처리한다.
- **본 wave 영역 정의**: WIP 본문 Goal·AC가 명시한 영역. 같은 도메인·
  같은 의사결정 단위면 본 wave 영역
- **분리 정당 조건**: 다른 도메인·다른 의사결정 단위·다른 시점에서만.
  본 wave 점검에서 발견한 자잘한 항목 분리 금지. 단, AC 자체가 달라져
  별도 실측이 필요하거나 합치면 본 wave 완료 기준을 흐리면 별도 wave 1개를
  허용한다.
- **자가 점검 환각 차단**: "본질 집중"·"강제 최소화" 명목으로 같은 영역
  sub-task 분리 시 P11 게이트 회피. LLM 자가 판단 신뢰 X — 본 wave 영역
  안이면 무조건 처리
- **C-P-S-AC 기준**: 분리 판단은 WIP의 C, Goal, AC가 정한 영역을 기준으로
  한다. 현재 C를 설명하는 데 필요한 동형 후보를 "후속"으로 빼면 완료 위장이다.

실측 사례: `docs/cps/cp_split_completion_p12.md` (P12 박제 직후 LLM이
정확히 본 패턴으로 위반).
