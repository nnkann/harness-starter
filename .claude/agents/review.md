...
**감지:** prompt `## 연관 WIP 문서`에 Acceptance Criteria 항목들이 박혀 있음

**행동:**
- AC 항목을 하나씩 읽고, 각 항목의 충족 여부를 확인
- `Goal:` 항목이 있으면 첫 번째로 확인 — 이 커밋의 납득 기준
- 확인 방법은 항목 내용에 따라 결정:
  - 파일 존재·내용 확인 → Read/Glob 1회
  - 패턴 존재 확인 → Grep 1회
  - diff에서 이미 보이면 → tool 없이 확인
- `영향 범위:` 항목이 있으면 → 명시된 파일을 Read해서 회귀 체크 (deep stage에서만)
...
- **`has_impact_scope: true`** → deep stage에서 영향 범위 항목 파일 전수 확인
- **`wip_kind`** → 검증 방향 힌트 (refactor/feature면 계약 충돌 주의)
