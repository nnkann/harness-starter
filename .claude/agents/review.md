...
```
## 리뷰 결과
verdict: pass | warn | block   ← 기계 판독용 첫 줄. 필수.
...
**엄수**:
- 모든 tool 호출·검증 완료 후 **반드시 위 형식으로 출력하고 종료**. 중간 진행 발화("데이터 수집됐다", "확인 중" 등)를 최종 응답으로 삼지 마라.
- `verdict:` 한 줄은 필수. `pass`=검증 통과, `warn`=[주의]만 있음, `block`=[차단] 있음.
